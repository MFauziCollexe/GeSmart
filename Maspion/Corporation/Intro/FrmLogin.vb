...
Public Class FrmLogin
    ...
    Private Sub BtnMasuk_Click_1(sender As System.Object, e As System.EventArgs) Handles BtnMasuk.Click
        Using dt = SelectCon.execute("SELECT * FROM USERS WITH (NOLOCK) WHERE ID_USER = '" + TxtID.Text + "' AND NAMA_USER = '" + TxtUsername.Text + "' AND PASSWORD = '" + SmartEngine.encrypt(TxtPassword.Text) + "' AND STATUS_AKTIF=1 AND HAK_USER=1", "frmLogin.BtnMasuk.Click.dt")
            If dt.Rows.Count > 0 Then
                GroupUser = dt.Rows(0).Item("id_group")
                HakRubahHarga = dt.Rows(0).Item("hak_harga")
                HakJabatan = dt.Rows(0).Item("JABATAN")

                dtHakAkses = SelectCon.execute("SELECT ITEM,LIHAT,BARU,EDIT,BATAL,HAPUS,CETAK,HAK_H,HEADER,HAK_GH,GROUP_HEADER FROM HAK_USER WHERE ID_GROUP='" & GroupUser & "'", "frmLogin.BtnMasuk.Click.DtHakAkses")
                ...
                Using dtperusahaan = SelectCon.execute("SELECT * FROM PERUSAHAAN", "frmLogin.BtnMasuk.Click.dtPerusahaan")
                ...
                Using dt2 = SelectCon.execute("SELECT * FROM BUKU_PERIODE WITH (NOLOCK) WHERE PERIODE = '" + Format(TglPeriode.Value, "MMyy") + "'", "frmLogin.BtnMasuk.Click.dt2")
                    If dt.Rows.Count > 0 Then
                        UserID = TxtID.Text
