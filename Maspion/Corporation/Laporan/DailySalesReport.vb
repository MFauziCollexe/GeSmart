...
Public Class DailySalesReport
    ...
    Friend WithEvents RGPilihCetakan As DevExpress.XtraEditors.RadioGroup
    ...
    Private Sub InitializeComponent()
        ...
        Me.RGPilihCetakan = New DevExpress.XtraEditors.RadioGroup()
        ...
        CType(Me.RGPilihCetakan.Properties, System.ComponentModel.ISupportInitialize).BeginInit()
        ...
        'BtnView
        '
        Me.BtnView.Location = New System.Drawing.Point(97, 356)
        ...
        Me.GBPilihan.Controls.Add(Me.RGPilihCetakan)
        ...
        Me.GBPilihan.Size = New System.Drawing.Size(329, 396)
        ...
        'LabelControl4
        '
        Me.LabelControl4.Location = New System.Drawing.Point(8, 335)
        ...
        Me.LabelControl4.Text = "Pilih Cetakan"
        Me.LabelControl4.Visible = False
        '
        'RGPilihCetakan
        '
        Me.RGPilihCetakan.AutoSizeInLayoutControl = True
        Me.RGPilihCetakan.EditValue = "Semua"
        Me.RGPilihCetakan.EnterMoveNextControl = True
        Me.RGPilihCetakan.Location = New System.Drawing.Point(97, 329)
        Me.RGPilihCetakan.Margin = New System.Windows.Forms.Padding(1)
        Me.RGPilihCetakan.Name = "RGPilihCetakan"
        Me.RGPilihCetakan.Properties.Items.AddRange(New DevExpress.XtraEditors.Controls.RadioGroupItem() {New DevExpress.XtraEditors.Controls.RadioGroupItem("Cetak Non Promo", "Cetak Non Promo"), New DevExpress.XtraEditors.Controls.RadioGroupItem("Cetak Ada Promo", "Cetak Ada Promo")})
        Me.RGPilihCetakan.Size = New System.Drawing.Size(217, 25)
        Me.RGPilihCetakan.TabIndex = 265
        Me.RGPilihCetakan.Visible = False
        '
        'Label6
        '
        ...
        CType(Me.RGPilihCetakan.Properties, System.ComponentModel.ISupportInitialize).EndInit()
    ...
    Private Sub ReportShowPreview()
        Dim MyReport = New XRDailySalesReportPromo
        ...
        If CmbKodePromo.SelectedItem IsNot "" Then
            ...
            promo = "AND DETAIL_NOTA.ID_BARANG IN (select ID_BARANG from LINK_BARANG_PROMO WITH (NOLOCK) WHERE CONVERT(DATE,'" & DtTanggalAwal.DateTime & "',103) >= CONVERT(DATE,TGL_AWAL,103) AND CONVERT(DATE,'" & DtTanggalAkhir.DateTime & "',103) <= CONVERT(DATE,TGL_AKHIR,103) AND KODE_PROMO='" & CmbKodePromo.SelectedItem.ToString.Split("-")(0).Trim & "')"
        ...
        Report.DataSource = SelectCon.execute("SELECT * FROM (SELECT DISTINCT ISNULL(LBP.KODE_PROMO,'')KODE_PROMO,A.NAMA,A.ID_TRANSAKSI, RIGHT(A.NO_NOTA, 6) AS NO_NOTA, A.NO_REF, A.TGL, A.TGL_PENGAKUAN, A.ID_DO, RIGHT(A.NO_DO, 6) AS NO_DO, A.TGL_DO, A.DIVISI, A.JENIS_PPN, A.KODE_CUSTOMER, A.KODE_APPROVE, A.CABANG, A.ALAMAT_KIRIM, A.GUDANG, A.SJ_TANPA_HARGA, A.KETERANGAN_CETAK, A.KETERANGAN_INTERNAL, A.SUBTOTAL, A.DPP, A.PPN, A.PERIODE, A.CRUSER, A.CRTIME, A.MDUSER, A.MDTIME, A.BATAL, A.BAGIAN, A.ID_STUFFING, A.KODE_EKSPEDISI, A.NO_TRUK, A.NAMA_DIVISI AS NAMA_DIVISI, A.NO_DO AS NO_DO1, A.PEMBAYARAN, A.DISC_QTY, A.DISC_QTY_NOMINAL, A.DISC_REG, A.DISC_REG_NOMINAL, A.DISC_1, A.DISC_1_NOMINAL, A.DISC_2, A.DISC_2_NOMINAL, A.DISC_3, A.CASH_DISC, A.DISC_3_NOMINAL, A.CASH_DISC_NOMINAL, A.DISC_QTY1, A.DISC_QTY_NOMINAL1, A.NAMA_GUDANG FROM ( SELECT DISTINCT DETAIL_NOTA.ID_BARANG, NOTA.ID_TRANSAKSI, RIGHT(NOTA.NO_NOTA, 6) AS NO_NOTA, NOTA.NO_REF, NOTA.TGL, NOTA.TGL_PENGAKUAN, NOTA.ID_DO, RIGHT(NOTA.NO_DO, 6) AS NO_DO, NOTA.TGL_DO, NOTA.DIVISI, NOTA.JENIS_PPN, NOTA.KODE_CUSTOMER, NOTA.KODE_APPROVE, NOTA.CABANG, NOTA.ALAMAT_KIRIM, NOTA.GUDANG, NOTA.SJ_TANPA_HARGA, NOTA.KETERANGAN_CETAK, NOTA.KETERANGAN_INTERNAL, NOTA.SUBTOTAL, NOTA.DPP, NOTA.PPN, NOTA.PERIODE, NOTA.CRUSER, NOTA.CRTIME, NOTA.MDUSER, NOTA.MDTIME, NOTA.BATAL, NOTA.BAGIAN, NOTA.ID_STUFFING, NOTA.KODE_EKSPEDISI, NOTA.NO_TRUK, IIF(ISNULL(NOTA.PRINT_PKP,0)=1,CUSTOMER.NAMA_PKP,CUSTOMER.NAMA) AS NAMA, DIVISI.NAMA AS NAMA_DIVISI, DELIVERY_ORDER.NO_DO AS NO_DO1, DELIVERY_ORDER.PEMBAYARAN, NOTA.DISC_QTY, NOTA.DISC_QTY_NOMINAL, NOTA.DISC_REG, NOTA.DISC_REG_NOMINAL, NOTA.DISC_1, NOTA.DISC_1_NOMINAL, NOTA.DISC_2, NOTA.DISC_2_NOMINAL, NOTA.DISC_3, NOTA.CASH_DISC, NOTA.DISC_3_NOMINAL, NOTA.CASH_DISC_NOMINAL, NOTA.DISC_QTY1, NOTA.DISC_QTY_NOMINAL1, GUDANG.NAMA_GUDANG FROM NOTA JOIN DETAIL_NOTA ON NOTA.ID_TRANSAKSI=DETAIL_NOTA.ID_TRANSAKSI LEFT OUTER JOIN GUDANG ON NOTA.GUDANG = GUDANG.KODE LEFT OUTER JOIN DELIVERY_ORDER ON NOTA.ID_DO = DELIVERY_ORDER.ID_TRANSAKSI LEFT OUTER JOIN DIVISI ON NOTA.DIVISI = DIVISI.KODE LEFT OUTER JOIN CUSTOMER ON NOTA.KODE_CUSTOMER = CUSTOMER.ID WHERE CONVERT(DATE,NOTA.TGL_PENGAKUAN,103) >= CONVERT(DATE,'" & DtTanggalAwal.DateTime & "',103) AND CONVERT(DATE,NOTA.TGL_PENGAKUAN,103) <= CONVERT(DATE,'" & DtTanggalAkhir.DateTime & "',103) " &
    filter &
    ...
    IIf(TxtKodeGudang.Text = "", "", " AND NOTA.GUDANG='" & TxtKodeGudang.Text & "' ") & promo & " ) A LEFT JOIN LINK_BARANG_PROMO LBP ON A.ID_BARANG=LBP.ID_BARANG ) Z ORDER BY Z.NAMA_DIVISI,Z.TGL_PENGAKUAN,Z.NAMA_GUDANG,CAST(RIGHT(Z.NO_NOTA, 6) AS INT) ")
        'IIf(TxtKodeGudang.Text = "", "", " AND NOTA.GUDANG='" & TxtKodeGudang.Text & "' ") & promo & " ) A LEFT JOIN LINK_BARANG_PROMO LBP ON A.ID_BARANG=LBP.ID_BARANG AND CONVERT(DATE,A.TGL,103) >= CONVERT(DATE,LBP.TGL_AWAL,103) AND CONVERT(DATE,A.TGL,103) <= CONVERT(DATE,LBP.TGL_AKHIR,103)) Z ORDER BY Z.NAMA_DIVISI,Z.TGL_PENGAKUAN,Z.NAMA_GUDANG,CAST(RIGHT(Z.NO_NOTA, 6) AS INT) ")
        ''08/04/2022 Mukhammad Fauzi (Modif)
    ...
    Private Sub LaporanBahan_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        Using dt = SelectCon.execute("SELECT KODE + ' - ' + NAMA_PROMO FROM PROMO with (NOLOCK)")
    ...
    Private Sub BtnView_Click(sender As System.Object, e As System.EventArgs) Handles BtnView.Click
        'If RGPilihCetakan.Text = "Cetak Ada Promo" Then
        ReportShowPreview()
        'Else
        '    ReportShowPreview2()
        'End If
