...
Public Class LaporanPenjualan
    ...
    Private Sub ReportShowPreviewRekapBruto()
        ...
        Report.DataSource = SelectCon.execute("SELECT DISTINCT REPLACE(NOTA.NO_NOTA,DIVISI.NAMA,'') AS NO_NOTA, 
            ...
            ISNULL(DO.KODE_GUDANG_CUSTOMER,'') AS KODE_GUDANG_CUSTOMER
        FROM BARANG WITH (NOLOCK) INNER JOIN NOTA INNER JOIN DETAIL_NOTA ON NOTA.ID_TRANSAKSI = DETAIL_NOTA.ID_TRANSAKSI ON BARANG.ID = DETAIL_NOTA.ID_BARANG 
            INNER JOIN (
                SELECT ID_TRANSAKSI, ID_PO, TGL_PENGAKUAN, SALES, PEMBAYARAN,ISNULL(KODE_GUDANG_CUSTOMER,'') AS KODE_GUDANG_CUSTOMER from DELIVERY_ORDER
                UNION ALL
                SELECT BON.ID_TRANSAKSI, DO.ID_PO, BON.TGL_PENGAKUAN, DO.SALES, DO.PEMBAYARAN,ISNULL(DO.KODE_GUDANG_CUSTOMER,'') AS KODE_GUDANG_CUSTOMER  
    ...
    Private Sub ReportShowPreviewRekapNetto()
        ...
        Report = MyReport
        Report.DataSource = SelectCon.execute("SELECT DISTINCT REPLACE(NOTA.NO_NOTA,DIVISI.NAMA,'') AS NO_NOTA, NOTA.BATAL, NOTA.NO_REF, NOTA.TGL, NOTA.TGL_PENGAKUAN, REPLACE(NOTA.NO_DO,DIVISI.NAMA,'') AS NO_DO, NOTA.ID_DO, CONVERT(VARCHAR,DO.TGL_PENGAKUAN,1) TGL_DO, NOTA.DIVISI, NOTA.GUDANG, NOTA.KODE_CUSTOMER, NOTA.KODE_APPROVE, CUSTOMER.NAMA AS NAMA_CUSTOMER, DIVISI.NAMA AS NAMA_DIVISI, BARANG.KODE, BARANG.NAMA AS NAMA_BARANG, DETAIL_NOTA.ISI, DETAIL_NOTA.KOLI, DETAIL_NOTA.QUANTITY, DETAIL_NOTA.SATUAN, DETAIL_NOTA.KONVERSI, DETAIL_NOTA.PCS, (((((((DETAIL_NOTA.HARGA-DETAIL_NOTA.DISKON_SATUAN)-NOTA.DISC_QTY)*(100-NOTA.DISC_REG)/100)*(100-NOTA.DISC_1)/100)*(100-NOTA.DISC_2)/100)*(100-NOTA.DISC_3)/100)*(100-NOTA.CASH_DISC)/100)*(100-NOTA.DISC_QTY1)/100 AS HARGA, USERS.NAMA_USER, DETAIL_NOTA.DISC, DETAIL_NOTA.DISKON_SATUAN, CORP.KODE_CORPORATION AS KODE_CORP,ISNULL(DO.KODE_GUDANG_CUSTOMER,'') AS KODE_GUDANG_CUSTOMER
        FROM BARANG INNER JOIN NOTA INNER JOIN DETAIL_NOTA ON NOTA.ID_TRANSAKSI = DETAIL_NOTA.ID_TRANSAKSI ON BARANG.ID = DETAIL_NOTA.ID_BARANG INNER JOIN 
            ( SELECT ID_TRANSAKSI, ID_PO, TGL_PENGAKUAN, SALES, PEMBAYARAN,ISNULL(KODE_GUDANG_CUSTOMER,'') AS KODE_GUDANG_CUSTOMER from DELIVERY_ORDER
            UNION ALL
            SELECT BON.ID_TRANSAKSI, DO.ID_PO, BON.TGL_PENGAKUAN, DO.SALES,DO.PEMBAYARAN,ISNULL(DO.KODE_GUDANG_CUSTOMER,'') AS KODE_GUDANG_CUSTOMER
            FROM DELIVERY_ORDER DO JOIN BON_PESANAN BON ON DO.ID_TRANSAKSI=BON.ID_DO
    ...
    Private Sub ReportShowPreviewEksport()
        ...
        Report.DataSource = SelectCon.execute("SELECT DISTINCT *, RTRIM(SUBSTRING(TEMP_PO, 0,
                    ...
                    " & cond & " ) Z
                UNION ALL
                SELECT DISTINCT *, RTRIM(SUBSTRING(TEMP_PO, 0,
					                    IIF(CHARINDEX(CHAR(13)+CHAR(10), TEMP_PO) = 0, 
						                    IIF(CHARINDEX(' ', TEMP_PO) = 0, 
							                    LEN(TEMP_PO)+1, 
							                    CHARINDEX(' ', TEMP_PO)), 
                    ...
                    SELECT	IIF(CUSTOMER.GROUP_CUSTOMER = 'Distributor', 'Distributor',
                            ...
                            LTRIM(REPLACE(REPLACE(
				                    SUBSTRING(NOTA.KETERANGAN_CETAK, 
					                    ...
					                    LEN(NOTA.KETERANGAN_CETAK))
				                    , ':', ''), ';', ' ')) TEMP_SUPP
                    FROM NOTA LEFT JOIN BON_PESANAN ON NOTA.ID_DO=BON_PESANAN.ID_TRANSAKSI LEFT JOIN DELIVERY_ORDER ON BON_PESANAN.ID_DO=DELIVERY_ORDER.ID_TRANSAKSI
                    LEFT JOIN CUSTOMER ON DELIVERY_ORDER.KODE_CUSTOMER=CUSTOMER.ID 
                    LEFT JOIN LINK_CORPORATION_CUSTOMER LC ON CUSTOMER.ID=LC.ID_CUSTOMER
                    LEFT JOIN CORPORATION ON CORPORATION.KODE=LC.KODE_CORPORATION
                    LEFT JOIN DIVISI ON DELIVERY_ORDER.DIVISI=DIVISI.KODE 
    ...
    Private Sub LaporanBahan_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        Using dt = SelectCon.execute("SELECT KODE + ' - ' + NAMA FROM GROUP_BARANG WITH (NOLOCK) ")
            For Each R In dt.Rows
    ...
    Private Sub TxtIDCustomer_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles LblIDCustomer.TextChanged
        Using dtCustomer = SelectCon.execute("SELECT KODE_APPROVE,NAMA FROM CUSTOMER WITH (NOLOCK) WHERE ID='" & LblIDCustomer.Text & "'")
    ...
    Private Sub LblIDBarang_TextChanged(sender As Object, e As System.EventArgs) Handles LblIDBarang.TextChanged
        Using dtBarang = SelectCon.execute("SELECT KODE,NAMA FROM BARANG WITH (NOLOCK) WHERE ID='" & LblIDBarang.Text & "'")
    ...
    Private Sub TxtIDDO_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtIDDO.EditValueChanged
        'Using data = SelectCon.execute("SELECT NO_DO FROM DELIVERY_ORDER WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDDO.Text & "'")
        ''06/04/2022 Mukhammad Fauzi (Modif)
        Using data = SelectCon.execute("select A.NO_DO from NOTA INNER JOIN (SELECT ID_TRANSAKSI, NO_DO from DELIVERY_ORDER UNION ALL SELECT BON.ID_TRANSAKSI, A.NO_DO FROM DELIVERY_ORDER A JOIN BON_PESANAN BON ON A.ID_TRANSAKSI=BON.ID_DO ) A ON NOTA.ID_DO=A.ID_TRANSAKSI where A.ID_TRANSAKSI ='" & TxtIDDO.Text & "'")
    ...
    Private Sub TxtKodeCorp_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtKodeCorp.EditValueChanged
        Using dtCustomer = SelectCon.execute("SELECT KODE,NAMA FROM CORPORATION WITH (NOLOCK) WHERE KODE='" & TxtKodeCorp.Text & "'")
            If dtCustomer.Rows.Count > 0 Then
