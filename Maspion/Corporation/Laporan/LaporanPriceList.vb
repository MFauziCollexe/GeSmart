...
Public Class LaporanPriceList
    ...
    Private Sub ReportShowPreview()
        ...
        If RadioGroup1.SelectedIndex = 0 Then
            Report.DataSource = SelectCon.execute("SELECT B.KODE,B.NAMA,A.ID_BARANG,A.JENIS,HARGA_BARU,CASE A.JENIS WHEN 'Langganan' THEN B.SAT_DIST1 WHEN 'Supermarket' THEN B.SAT_SUPER1 END AS SATUAN,QTY_KOLI,D.NAMA,A.TGL_PRICE FROM VI_PRICE_LIST A WITH (NOLOCK) LEFT JOIN BARANG B WITH (NOLOCK) ON A.ID_BARANG=B.ID LEFT JOIN LINK_BARANG_DIVISI C ON B.ID=C.ID_BARANG LEFT JOIN DIVISI D WITH (NOLOCK) ON C.KODE_DIVISI=D.KODE WHERE B.STATUS_AKTIF=1 " & IIf(TxtDivisi.Text = "", "", " AND D.KODE='" & TxtDivisi.Text & "' ") & IIf(DtTanggal.Text = "", "", " AND CONVERT(DATE,A.TGL_PRICE,103) = CONVERT(DATE,'" & DtTanggal.DateTime & "',103) ") & IIf(RDPenjualan.SelectedIndex = -1 Or RDPenjualan.SelectedIndex = 0, "", IIf(RDPenjualan.SelectedIndex = 3, " AND A.ID <> 'UMUM' ", " AND A.JENIS LIKE '%" & RDPenjualan.EditValue & "%' ")))
        Else
            Report.DataSource = SelectCon.execute("SELECT URUTAN,B.KODE,B.NAMA,A.ID_BARANG,A.JENIS,HARGA_BARU,CASE A.JENIS WHEN 'Langganan' THEN B.SAT_DIST1 WHEN 'Supermarket' THEN B.SAT_SUPER1 END AS SATUAN,QTY_KOLI,D.NAMA,A.TGL_PRICE FROM (SELECT ID_BARANG,TGL_PRICE,JENIS,ID,HARGA_BARU,ROW_NUMBER() OVER(PARTITION BY ID_BARANG ORDER BY TGL_PRICE DESC) AS URUTAN FROM VI_PRICE_LIST WITH (NOLOCK) WHERE CONVERT(DATE,TGL_PRICE,103) <= CONVERT(DATE,'" & DtTglTerakhir.DateTime & "',103) " & IIf(RDPenjualan.SelectedIndex = -1 Or RDPenjualan.SelectedIndex = 0, "", IIf(RDPenjualan.SelectedIndex = 3, " AND ID <> 'UMUM' ", " AND JENIS LIKE '%" & RDPenjualan.EditValue & "%' ")) & ") A LEFT JOIN BARANG B WITH (NOLOCK) ON A.ID_BARANG=B.ID LEFT JOIN LINK_BARANG_DIVISI C WITH (NOLOCK) ON B.ID=C.ID_BARANG LEFT JOIN DIVISI D WITH (NOLOCK) ON C.KODE_DIVISI=D.KODE WHERE A.URUTAN=1 AND B.STATUS_AKTIF=1 " & IIf(TxtDivisi.Text = "", "", " AND D.KODE='" & TxtDivisi.Text & "' ") & " ORDER BY TGL_PRICE")
        End If
