Public MustInherit Class FrmBarang
    Public dtDivisi As New DataTable
    Public dtPromo As New DataTable
    ...
    Private Sub _Toolbar1_Button6_Click(sender As Object, e As EventArgs) Handles _Toolbar1_Button6.Click
...
keluar:
    ...
    Private Sub Hapus() Handles _Toolbar1_Button4.Click
        ...
        If con.exec(SQl) Then
            ...
            If con.exec("DELETE FROM LINK_BARANG_PROMO WHERE ID_BARANG='" & TxtIDBarang.Text & "'") = False Then GoTo keluar
            con.end_exec(True)
            MessageBox.Show("Data Baru telah dihapus..!!",
                            "Penyimpanan Sukses",
...
keluar:
    ...
    Private Sub _Toolbar1_Button6_Click(sender As System.Object, e As System.EventArgs) Handles _Toolbar1_Button6.Click
        Dim frm As New InputBarang
        frm.MdiParent = Me.MdiParent
        frm.Show()
        frm.Focus()
        Dim ctl() As Control
        For Each MyControl In frm.Controls
            ctl = Me.Controls.Find(MyControl.Name, True)
            SetControlValue(MyControl, ctl)
        Next
        For Each MyControl In frm.GroupControl1.Controls
            ctl = Me.GroupControl1.Controls.Find(MyControl.Name, True)
            SetControlValue(MyControl, ctl)
        Next

        CopyDataTable(frm.dtDivisi, Me.dtDivisi)
        CopyDataTable(frm.dtPromo, Me.dtPromo)
    End Sub

    Sub SetControlValue(ByRef MyControl, ByRef ctl)
        Select Case TypeName(MyControl)
            Case "TextEdit", "TextEdit", "MemoEdit", "ButtonEdit", "CalcEdit"
                Try : MyControl.Text = ctl(0).Text
                Catch : End Try
            Case "ComboBoxEdit"
                Try : DirectCast(MyControl, DevExpress.XtraEditors.ComboBoxEdit).SelectedItem =
                    DirectCast(ctl(0), DevExpress.XtraEditors.ComboBoxEdit).SelectedItem
                Catch : End Try
            Case "CheckEdit"
                Try : DirectCast(MyControl, DevExpress.XtraEditors.CheckEdit).Checked =
                    DirectCast(ctl(0), DevExpress.XtraEditors.CheckEdit).Checked
                Catch : End Try
            Case "DateEdit"
                Try : DirectCast(MyControl, DevExpress.XtraEditors.DateEdit).DateTime =
                    DirectCast(ctl(0), DevExpress.XtraEditors.DateEdit).DateTime
                    DirectCast(MyControl, DevExpress.XtraEditors.DateEdit).Refresh()
                Catch : End Try
            Case "RadioGroup"
                Try : DirectCast(MyControl, DevExpress.XtraEditors.RadioGroup).EditValue =
