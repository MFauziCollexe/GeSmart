...
Public Class MenuKendaraan
    ...
    Private Sub GetData()
        TBMenu.DataSource = SelectCon.execute("SELECT KODE_KENDARAAN, NOPOL, JENIS, MERK, TIPE, WARNA, STATUS_AKTIF FROM KENDARAAN ORDER BY KODE_KENDARAAN")
        TBMenu.Refresh()
        Frame1.Text = "[ " & GridView1.DataRowCount & " Data Kendaraan ]"
        GridView1.Columns(0).Caption = "Kode"
        GridView1.Columns(0).Width = 50
    ...
    Private Sub MenuKaryawan_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        Key = "Kendaraan"
        AddHandler Me.Activated, AddressOf GetData
        HakMenu("", "Master", "Kendaraan", BarButtonBaru, {BtnEdit, BarButtonEdit}, BtnCetak)
    End Sub

    Private Sub Baru() Handles BarButtonBaru.ItemClick
        InputKendaraan.MdiParent = Me.MdiParent
        InputKendaraan.Show()
        InputKendaraan.Focus()
    End Sub

    Private Sub Edit() Handles BarButtonEdit.ItemClick
        If GridView1.RowCount > 0 Then
            If Not IsDBNull(GridView1.GetFocusedRow(0)) Then
                EditKendaraan.MdiParent = Me.MdiParent
                EditKendaraan.Show()
                EditKendaraan.Focus()
                EditKendaraan.TxtKode.Text = GridView1.GetFocusedRow(0)
            End If
        End If
    End Sub
...
'============
Public Class MenuGudangCustomer
    Inherits FrmMenu

    Private Sub GetData()
        TBMenu.DataSource = SelectCon.execute("SELECT KODE_GUDANG_CUSTOMER,NAMA_GUDANG,ALAMAT_GUDANG,KOTA_GUDANG,TELP_GUDANG,PENANGGUNG_JAWAB,STATUS_AKTIF FROM GUDANG_CUSTOMER ORDER BY KODE_GUDANG_CUSTOMER")
        TBMenu.Refresh()
        Frame1.Text = "[ " & GridView1.DataRowCount & " Data Gudang Customer ]"
        GridView1.Columns(0).Caption = "Kode"
        GridView1.Columns(0).Width = 50
        GridView1.Columns(1).Caption = "Nama Gudang"
        GridView1.Columns(1).Width = 125
        ...
        GridView1.Columns(4).Caption = "Tlp"
        GridView1.Columns(4).Width = 75
        GridView1.Columns(5).Caption = "Penanggung Jawab"
        GridView1.Columns(5).Width = 75
        GridView1.Columns(6).Caption = "Status Aktif"
        GridView1.Columns(6).Width = 75
    End Sub
    ...
    Private Sub MenuKaryawan_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        Key = "Gudang Customer"
        AddHandler Me.Activated, AddressOf GetData
        HakMenu("", "Master", "Gudang Customer", BarButtonBaru, {BtnEdit, BarButtonEdit}, BtnCetak)
    End Sub

    Private Sub Baru() Handles BarButtonBaru.ItemClick
        InputGudangCustomer.MdiParent = Me.MdiParent
        InputGudangCustomer.Show()
        InputGudangCustomer.Focus()
    End Sub

    Private Sub Edit() Handles BarButtonEdit.ItemClick
        If GridView1.RowCount > 0 Then
            If Not IsDBNull(GridView1.GetFocusedRow(0)) Then
                EditGudangCustomer.MdiParent = Me.MdiParent
                EditGudangCustomer.Show()
                EditGudangCustomer.Focus()
                EditGudangCustomer.TxtKode.Text = GridView1.GetFocusedRow(0)
            End If
        End If
    End Sub
    ...
    Private Sub BtnLihat_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BtnLihat.ItemClick
        If GridView1.RowCount > 0 Then
            If Not IsDBNull(GridView1.GetFocusedRow(0)) Then
                Dim frm As New EditGudangCustomer
                frm.MdiParent = Me.MdiParent
                frm.Show()
                frm.Focus()
                frm.Text = Replace(frm.Text, "Edit", "Lihat")
