...
Public Class SQLServer
    ...
    Public Sub Dispose() Implements IDisposable.Dispose
        ...
        CloseConn()
        If conn IsNot Nothing Then conn.Dispose() : conn = Nothing
        If cmd IsNot Nothing Then cmd.Dispose() : cmd = Nothing
        If Da IsNot Nothing Then Da.Dispose() : Da = Nothing
        If transaction IsNot Nothing Then transaction.Dispose() : transaction = Nothing
    ...
    Public Function Sconnect() As Boolean
        ...
        query = "Server=" & astring(0) & ";Database=" & astring(1) & ";User Id=sa;password=Gsmart16816899;Connection Timeout=30;Application Name=ExecFrom_GsmartDesktop_"
        Try
...
Public Class SelectSQLServer
    ...
    Public Function Sconnect(ByVal SenderName As String) As Boolean
        SenderName = If(SenderName, "")

        Try
            Dim astring As String() = TxtServer.Split(";".ToCharArray(), StringSplitOptions.RemoveEmptyEntries)
            Dim Query As String = "Server=" & astring(0) & ";Database=" & astring(1) & ";User Id=sa;password=Gsmart16816899;Connection Timeout=30;Application Name=ExecFrom_GsmartDesktop_" & "_@" & SenderName
    ...
    'Public Function execute(ByVal Query As String, Optional ByVal SenderName As String = "") As DataTable
    '    Try
    '        If Sconnect(SenderName) Then
    ...
    Public Function execute(ByVal Query As String, Optional ByVal SenderName As String = "") As DataTable
        Try
            If execute(Dt, Query, SenderName) Then
                If Dt IsNot Nothing Then
                    'MsgBox("Query Gagal..!!", MsgBoxStyle.Critical, "Query Failed")
                End If
            Else
                MsgBox("Koneksi Gagal..!!", MsgBoxStyle.Critical, "Access Failed")
                Dt = Nothing
    ...
    Public Function execute(ByRef Datatables As DataTable, ByVal Query As String, Optional ByVal SenderName As String = "") As Boolean
        Dim Result As Boolean = False

        Dim astring As String() = TxtServer.Split(";".ToCharArray(), StringSplitOptions.RemoveEmptyEntries)
        Dim StrKonSql As String = "Server=" & astring(0) & ";Database=" & astring(1) & ";User Id=sa;password=Gsmart16816899;Connection Timeout=30;Application Name=New_SelectSQLServer_execute_"

        Using conn = New SqlConnection(StrKonSql)
            Using cmd = New SqlCommand(Query, conn)
                Using Da = New SqlDataAdapter(cmd)
                    Try
                        ...
                        cmd.CommandText = Query
                        Da.SelectCommand = cmd
                        If Datatables IsNot Nothing Then Datatables.Dispose() : Datatables = Nothing
