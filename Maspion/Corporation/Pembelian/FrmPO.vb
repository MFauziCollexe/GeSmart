...
#End Region
    ...
    Private Sub TxtIDCustomer_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtIDCustomer.EditValueChanged
        ...
        Using dtCustomer = SelectCon.execute("SELECT KODE_APPROVE,NAMA,LOKASI_PENGIRIMAN,SYARAT_PEMBAYARAN_KREDIT FROM CUSTOMER WITH (NOLOCK) WHERE ID='" & TxtIDCustomer.Text & "'")
    ...
    Private Sub TxtDivisi_EditValueChanged(sender As Object, e As System.EventArgs) Handles TxtDivisi.EditValueChanged
        ...
        SetData("SELECT DISTINCT A.KODE_SBU,B.NAMA FROM LINK_SBU_DIVISI A WITH (NOLOCK) INNER JOIN SBU B WITH (NOLOCK) ON A.KODE_SBU=B.KODE WHERE A.KODE_DIVISI='" & TxtDivisi.Text & "'", {TxtIDSupplier, TxtNamaSupplier})
        SetData("SELECT A.ID_CUSTOMER FROM SETUP_CUSTOMER_PEMBELIAN A WITH (NOLOCK) JOIN PT B WITH (NOLOCK) ON A.KODE_PT=B.KODE JOIN LINK_PT_SBU C WITH (NOLOCK) ON B.KODE=C.KODE_PT JOIN LINK_SBU_DIVISI D WITH (NOLOCK) ON C.KODE_SBU=D.KODE_SBU WHERE D.KODE_DIVISI='" & TxtDivisi.Text & "'", {TxtIDCustomer})
...
DiskonQty2:             TempTotalDiskon = CDbl(TxtDiskonQty1Nominal.Value) + CDbl(TxtDiskonRegulerNominal.Value) + CDbl(TxtDiskon1Nominal.Value) + _
    ...
    Private Sub BtnRubahHarga_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BtnRubahHarga.ItemClick
        Using LoadDataToGrid As New _LoadDataToGrid
            LoadDataToGrid.BeginInit("SELECT 'Pilih' AS PILIH,HARGA_BARU,TGL_PRICE FROM VI_PRICE_LIST WITH (NOLOCK) WHERE ID_BARANG='" & GridView1.GetFocusedDataRow()("ID Barang") & "' AND JENIS='Langganan' AND (ID='UMUM' OR ID='" & TxtIDCustomer.Text & "') ORDER BY TGL_PRICE DESC")
    ...
    Private Sub BtnPilihDO_Click(sender As System.Object, e As System.EventArgs) Handles BtnPilihDO.Click
        ...
        If kode <> "" Then
            ...
            If EnumDescription(Bagian).Contains("Supermarket") Then
                Query = "SELECT A.URUTAN [No.],A.ID_BARANG [ID Barang],B.KODE [Kode Barang],B.NAMA [Nama Barang],B.SAT_DIST1 SATUAN,B.QTY_KOLI ISI,A.KOLI AS KOLI,A.QUANTITY/B.QTY_DIST [Kuantum],A.PCS [Pieces],B.QTY_DIST [Konv],B.HARGA_DIST [Harga Satuan],A.DISC [Disc %],A.DISKON_SATUAN [Disc Satuan],ROUND(A.QUANTITY * (A.HARGA - A.DISKON_SATUAN),2) AS SUBTOTAL,A.ID_TRANSAKSI [ID DO],C.NO_DO [No. DO] FROM V_D_DB_PERW_T_PO A WITH (NOLOCK) LEFT JOIN BARANG B WITH (NOLOCK) ON A.ID_BARANG=B.ID LEFT JOIN DELIVERY_ORDER C ON A.ID_TRANSAKSI=C.ID_TRANSAKSI WHERE A.ID_TRANSAKSI='" & kode & "' ORDER BY A.URUTAN"
            Else
                Query = "SELECT A.URUTAN [No.],A.ID_BARANG [ID Barang],B.KODE [Kode Barang],B.NAMA [Nama Barang],A.SATUAN,A.ISI,A.KOLI,A.QUANTITY [Kuantum],A.PCS [Pieces],A.KONVERSI [Konv],A.HARGA [Harga Satuan],A.DISC [Disc %],A.DISKON_SATUAN [Disc Satuan],ROUND(A.QUANTITY * (A.HARGA - A.DISKON_SATUAN),2) AS SUBTOTAL,A.ID_TRANSAKSI [ID DO],C.NO_DO [No. DO] FROM V_D_DB_PERW_T_PO A WITH (NOLOCK) LEFT JOIN BARANG B WITH (NOLOCK) ON A.ID_BARANG=B.ID LEFT JOIN DELIVERY_ORDER C ON A.ID_TRANSAKSI=C.ID_TRANSAKSI WHERE A.ID_TRANSAKSI='" & kode & "' ORDER BY A.URUTAN"
            End If

            Using Mydt As DataTable = SelectCon.execute(Query)
                If Mydt.Rows.Count > 0 Then
                    txtKeterangan.Text = txtKeterangan.Text & SelectCon.execute("SELECT A.NO_DO + ' - ' + B.NAMA FROM DELIVERY_ORDER A WITH (NOLOCK) LEFT JOIN CUSTOMER B WITH (NOLOCK) ON A.KODE_CUSTOMER=B.ID WHERE A.ID_TRANSAKSI='" & kode & "'")(0)(0) & vbCrLf
    ...
    Private Sub TxtIDSupplier_EditValueChanged(sender As Object, e As EventArgs) Handles TxtIDSupplier.EditValueChanged
