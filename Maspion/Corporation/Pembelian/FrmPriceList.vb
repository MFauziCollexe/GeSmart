...
#End Region
    ...
    Private Sub ReBEdit_ButtonClick(sender As Object, e As System.EventArgs) Handles ReBEdit.ButtonClick
        If GridView1.FocusedColumn.FieldName = "Kode Barang" Then
            ...
            'CEK DATA ADA / TIDAK 
            Using dt_cari = SelectCon.execute("SELECT TOP 1 A.ID, A.KODE, A.NAMA, " & satuan & ", A.SAT_SUPER1, A.SAT_SUPER2 FROM BARANG A WITH (NOLOCK) LEFT JOIN LINK_BARANG_DIVISI D WITH (NOLOCK) ON A.ID=D.ID_BARANG LEFT JOIN VI_PRICE_LIST B WITH (NOLOCK) ON A.ID=B.ID_BARANG WHERE (A.ID='" & col("Kode Barang") & "' OR A.KODE='" & col("Kode Barang") & "') AND A.STATUS_AKTIF=1 " & filt & " AND D.KODE_DIVISI='" & TxtDivisi.Text & "' ORDER BY TGL_PRICE DESC")
...
CariData:
            ...
            Else
                ...
                End If
            End If
            Using dt_cari = SelectCon.execute("SELECT TOP 1 A.ID, A.KODE, A.NAMA, " & satuan & ", A.SAT_SUPER1, A.SAT_SUPER2 FROM BARANG A WITH (NOLOCK) LEFT JOIN LINK_BARANG_DIVISI D WITH (NOLOCK) ON A.ID=D.ID_BARANG LEFT JOIN VI_PRICE_LIST B ON A.ID=B.ID_BARANG WHERE (A.ID='" & col("Kode Barang") & "' OR A.KODE='" & col("Kode Barang") & "') AND A.STATUS_AKTIF=1 " & filt & "AND D.KODE_DIVISI='" & TxtDivisi.Text & "' ORDER BY TGL_PRICE DESC")
                If dt_cari.Rows.Count = 1 Then
                    col("No.") = GridView1.FocusedRowHandle + 1
                    col("ID Barang") = dt_cari.Rows(0).Item("ID")
                    col("Kode Barang") = dt_cari.Rows(0).Item("KODE")
    ...
    Private Sub RBEditCust_ButtonClick(sender As Object, e As EventArgs) Handles RBEditCust.ButtonClick
        ...
        'CEK DATA ADA / TIDAK 
        Using dt_cari = SelectCon.execute("SELECT ID,KODE_APPROVE,NAMA FROM CUSTOMER A WITH (NOLOCK) WHERE A.KODE_APPROVE='" & col("Kode Customer") & "' " & filter)
...
CariData:
        ...
        Else
            ...
            End If
        End If
        Using dt_cari = SelectCon.execute("SELECT ID,KODE_APPROVE,NAMA FROM CUSTOMER A WITH (NOLOCK) WHERE A.ID='" & kode & "' " & filter)
            If dt_cari.Rows.Count > 0 Then
                col("ID Customer") = dt_cari.Rows(0).Item("ID")
                col("Kode Customer") = dt_cari.Rows(0).Item("KODE_APPROVE")
                col("Nama Customer") = dt_cari.Rows(0).Item("NAMA")
    ...
    Private Sub _Toolbar1_Button7_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles _Toolbar1_Button7.ItemClick
        ...
        With frm
            ...
            .Show()
            LoadData.GetData("SELECT A.URUTAN,A.ID_BARANG,B.KODE,B.NAMA,A.SATUAN,A.SATUAN1,A.ISI,A.SATUANK,A.SATUANK1,A.HARGA_LAMA,A.HARGA_BARU FROM DETAIL_PRICE_LIST A WITH (NOLOCK) LEFT JOIN BARANG B WITH (NOLOCK) ON A.ID_BARANG=B.ID WHERE A.ID_TRANSAKSI='" & TxtIDTransaksi.Text & "' ORDER BY A.URUTAN")
            LoadData.SetDataDetail(frm.dt, True)
            LoadData.GetData("SELECT B.ID,B.KODE_APPROVE,A.NAMA FROM CUSTOMER A WITH (NOLOCK) INNER JOIN DETAIL_PRICE_LIST_CUSTOMER B WITH (NOLOCK) ON A.ID=B.ID WHERE B.ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'")
