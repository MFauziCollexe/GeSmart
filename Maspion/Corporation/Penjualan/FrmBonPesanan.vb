...
#Region "Shared Sub"
    ...
    Protected Sub Generate()
        Dim urut As Short
        Using dtGenerate = SelectCon.execute("SELECT NO_BON FROM BON_PESANAN WITH (NOLOCK) WHERE NO_BON Like '" & TxtNamaDivisi.Text & "(T)" & "(" & InisialPerusahaan & ")" & "%' AND YEAR(TGL_PENGAKUAN)=" & Format(TglPengakuan.EditValue, "yyyy") & " ORDER BY NO_BON DESC")
    ...
    Protected Sub Simpan()
        ...
        If TxtSisa.EditValue < 0 Then
            If MessageBox.Show("Sisa tunai nomor DO titipan ini tidak mencukupi, apakah anda ingin melanjutkan ?", "Sisa tunai tidak mencukupi !", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No Then
                Exit Sub
            End If
        End If
...
#End Region
    ...
    Private Sub TxtKode_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtIDCustomer.EditValueChanged
        Using dtCustomer = SelectCon.execute("SELECT KODE_APPROVE,NAMA,ALAMAT_KANTOR,LOKASI_PENGIRIMAN,SYARAT_PEMBAYARAN_KREDIT FROM CUSTOMER WITH (NOLOCK) WHERE ID='" & TxtIDCustomer.Text & "'")
    ...
    Private Sub LoadDO()
        LoadData.GetData("SELECT [NO_DO] ,[TGL_PENGAKUAN] ,[TGL_PRICE] ,[DIVISI] ,'Include' ,[KODE_CUSTOMER] ,[KODE_APPROVE] ,[ALAMAT_KIRIM] ,[DISC_REG] ,[DISC_1] ,[DISC_2] ,[DISC_3] ,[CASH_DISC] FROM DELIVERY_ORDER WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDDO.Text & "'")
    ...
    Private Sub TxtIDDO_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtIDDO.EditValueChanged
        If Status_Edit = False Then
            SetData("SELECT TITIP-BON AS SALDO FROM V_TITIPAN_TERPENUHI_BON WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDDO.Text & "'", {TxtSaldo})
            If TxtSaldo.EditValue < 0 Then
                If MessageBox.Show("Sisa tunai nomor DO titipan ini tidak mencukupi, apakah anda ingin melanjutkan ?", "Sisa tunai tidak mencukupi !", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                    LoadDO()
                Else
...
CariData:
            ...
            Else
                ...
                End If
            End If
            'SELECT TOP 1 A.ID,A.KODE,A.NAMA," & IIf(EnumDescription(Bagian).Contains("Langganan"), "A.SAT_DIST1 AS SATUAN ,ISNULL(C.HARGA_BARU,A.HARGA_DIST) AS HARGA, A.QTY_KOLI AS ISI ", "A.SAT_SUPER1 AS SATUAN ,ISNULL(C.HARGA_BARU,A.HARGA_SUPER) AS HARGA, A.QTY_DIST AS ISI ") & " ,B.ISI AS KONV,C.TGL_AWAL TGL_PRICE FROM BARANG A INNER JOIN V_SATUAN_TO_PCS B ON A.ID=B.ID AND B.SATUAN1=" & IIf(EnumDescription(Bagian).Contains("Langganan"), " A.SAT_DIST1", " A.SAT_SUPER1") & " LEFT JOIN VI_PRICE_LIST C ON A.ID=C.ID_BARANG " & IIf(EnumDescription(Bagian).Contains("Langganan"), " AND C.JENIS='Langganan'", " AND C.JENIS='Supermarket'") & "AND (C.ID='" & TxtIDCustomer.Text & "' OR C.ID='UMUM') AND CAST(C.TGL_PRICE AS DATE) <= CONVERT(DATE,'" & Format(TglPriceList.DateTime, "MM-dd-yyyy") & "')" & " LEFT JOIN LINK_BARANG_DIVISI D ON A.ID=D.ID_BARANG WHERE A.ID='" & col("ID Barang") & "' " & " AND A.STATUS_AKTIF=1 AND D.KODE_DIVISI='" & TxtDivisi.Text & "' ORDER BY C.TGL_PRICE DESC,C.ID"
            Using dt_cari = SelectCon.execute("SELECT TOP 1 A.ID,A.KODE,A.NAMA," & IIf(EnumDescription(Bagian).Contains("Langganan"), "A.SAT_DIST1 AS SATUAN ,ISNULL(C.HARGA_BARU,A.HARGA_DIST) AS HARGA, A.QTY_KOLI AS ISI ", "A.SAT_SUPER1 AS SATUAN ,ISNULL(C.HARGA_BARU,A.HARGA_SUPER) AS HARGA, A.QTY_DIST AS ISI ") & " ,B.ISI AS KONV,C.TGL_PRICE FROM BARANG A INNER JOIN V_SATUAN_TO_PCS B ON A.ID=B.ID AND B.SATUAN1=" & IIf(EnumDescription(Bagian).Contains("Langganan"), " A.SAT_DIST1", " A.SAT_SUPER1") & " LEFT JOIN VI_PRICE_LIST C ON A.ID=C.ID_BARANG " & IIf(EnumDescription(Bagian).Contains("Langganan"), " AND C.JENIS='Langganan'", " AND C.JENIS='Supermarket'") & "AND (C.ID='" & TxtIDCustomer.Text & "' OR C.ID='UMUM') AND CAST(C.TGL_PRICE AS DATE) <= CONVERT(DATE,'" & Format(TglPriceList.DateTime, "MM-dd-yyyy") & "')" & " LEFT JOIN LINK_BARANG_DIVISI D ON A.ID=D.ID_BARANG WHERE A.ID='" & col("ID Barang") & "' " & " AND A.STATUS_AKTIF=1 AND D.KODE_DIVISI='" & TxtDivisi.Text & "' ORDER BY C.TGL_PRICE DESC,C.ID")
                If dt_cari.Rows.Count > 0 Then
                    col("No.") = GridView1.FocusedRowHandle + 1
                    col("Kode Barang") = dt_cari.Rows(0).Item("KODE")
...
#End Region
    ...
    Private Sub BtnRubahHarga_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BtnRubahHarga.ItemClick
        Using LoadDataToGrid As New _LoadDataToGrid
            LoadDataToGrid.BeginInit("SELECT 'Pilih' AS PILIH,HARGA_BARU,TGL_PRICE FROM VI_PRICE_LIST WITH (NOLOCK) WHERE ID_BARANG='" & GridView1.GetFocusedDataRow()("ID Barang") & "' AND JENIS='" & IIf(EnumDescription(Bagian).Contains("Supermarket"), "Supermarket", "Langganan") & "' AND (ID='UMUM' OR ID='" & TxtIDCustomer.Text & "') ORDER BY TGL_PRICE DESC")
    ...
    Private Sub TxtDOTitipan_EditValueChanged(sender As Object, e As EventArgs) Handles TxtDOTitipan.EditValueChanged
