...
#Region "Shared Sub"
    ...
    Protected Sub Generate()
        ...
        Using dtGenerate = SelectCon.execute("SELECT NO_DO FROM DELIVERY_ORDER WITH (NOLOCK) WHERE NO_DO LIKE '" & Filter & "%' AND YEAR(TGL_PENGAKUAN)='" & Format(TglPengakuan.EditValue, "yyyy") & "'" & Pusat & Super & " AND DIVISI='" & TxtDivisi.Text & "' ORDER BY NO_DO DESC")
        ...
        Using dtGenerate = SelectCon.execute("SELECT ISNULL(MAX(CAST(REPLACE(ID_TRANSAKSI,'DO','') AS BIGINT)),0) AS ID FROM DELIVERY_ORDER WITH (NOLOCK)")
    ...
    Protected Sub GetData(sender As Object, e As System.EventArgs)
        If TxtIDTransaksi.Text <> "" Then
            ...
            If TxtIDPO.Text <> "" Then
                Str_MyQuery = "SELECT DISTINCT B.URUTAN,B.ID_BARANG,BB.KODE,BB.NAMA,B.SATUAN,B.ISI,(AA.KOLI-AA.KOLI_T)+B.KOLI,(AA.QUANTITY-AA.QUANTITY_T)+B.QUANTITY,(AA.PCS-AA.PCS_T)+B.PCS,B.KOLI,B.QUANTITY,B.PCS,B.KONVERSI,B.HARGA,B.DISC,B.DISKON_SATUAN,ROUND(B.QUANTITY * (B.HARGA - B.DISKON_SATUAN),2) AS SUBTOTAL FROM (DELIVERY_ORDER A WITH (NOLOCK) INNER JOIN DETAIL_DELIVERY_ORDER B WITH (NOLOCK) ON A.ID_TRANSAKSI=B.ID_TRANSAKSI) LEFT JOIN V_D_PO_T_DO AA WITH (NOLOCK) ON A.ID_PO=AA.ID_TRANSAKSI AND B.ID_BARANG=AA.ID_BARANG LEFT JOIN BARANG BB WITH (NOLOCK) ON B.ID_BARANG=BB.ID WHERE A.ID_TRANSAKSI='" & TxtIDTransaksi.Text & "' ORDER BY URUTAN"
                LoadData.GetData(Str_MyQuery)
                LoadData.SetDataDetail(dt, False)
            Else
                Str_MyQuery = "SELECT A.URUTAN,A.ID_BARANG,B.KODE,B.NAMA,A.SATUAN,A.ISI,0,0,0,A.KOLI,A.QUANTITY,A.PCS,A.KONVERSI,A.HARGA,A.DISC,A.DISKON_SATUAN,ROUND(A.QUANTITY * (A.HARGA - A.DISKON_SATUAN),2) AS SUBTOTAL FROM DETAIL_DELIVERY_ORDER A WITH (NOLOCK) LEFT JOIN BARANG B WITH (NOLOCK) ON A.ID_BARANG=B.ID WHERE A.ID_TRANSAKSI='" & TxtIDTransaksi.Text & "' ORDER BY A.URUTAN"
                LoadData.GetData(Str_MyQuery)
    ...
    Protected Sub SimpanEdit(sender As Object, e As System.EventArgs)
        ...
        Using dtcek As DataTable = SelectCon.execute("SELECT STATUS_LUNAS AS BAYAR FROM VI_PAY_KONTAN WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'")
            If dtcek.Rows.Count > 0 Then
                If dtcek.Rows(0).Item(0) = True Then
                    MsgBox("Transaksi Sudah Dalam Proses Payment," & vbCrLf &
        ...
        If Bagian = EBagian.Lain_Perwakilan Or Bagian = EBagian.Langganan_Perwakilan Or Bagian = EBagian.Supermarket_Perwakilan Then
            Using dtcek As DataTable = SelectCon.execute("SELECT NO_STUFFING FROM STUFFING WITH (NOLOCK) WHERE ID_DO='" & TxtIDTransaksi.Text & "' AND BATAL=0")
                If dtcek.Rows.Count > 0 Then
                    MsgBox("Transaksi Sudah Dalam Proses Stuffing, No. Stuffing : " & dtcek.Rows(0)(0) & vbCrLf &
                           "Anda Tidak Dapat Mengubah Transaksi Ini !!" & vbCrLf &
        ...
        ElseIf Bagian = EBagian.Lain_Pusat Or Bagian = EBagian.Langganan_Pusat Or Bagian = EBagian.Supermarket_Pusat Then
            Using dtcek As DataTable = SelectCon.execute("SELECT NO_NOTA FROM NOTA WITH (NOLOCK) WHERE ID_DO='" & TxtIDTransaksi.Text & "' AND BATAL=0")
                If dtcek.Rows.Count > 0 Then
                    MsgBox("Transaksi Sudah Dalam Proses Nota / SJ, No. Nota/SJ : " & dtcek.Rows(0)(0) & vbCrLf &
                           "Anda Tidak Dapat Mengubah Transaksi Ini !!" & vbCrLf &
...
#End Region
    ...
    Private Sub TxtDivisi_EditValueChanged(sender As Object, e As System.EventArgs) Handles TxtDivisi.EditValueChanged
        ...
        SetData("SELECT NAMA FROM DIVISI WITH (NOLOCK) WHERE KODE='" & TxtDivisi.Text & "'", {TxtNamaDivisi})
        AllowGrid()
        ...
        If Bagian = EBagian.Lain_Perwakilan Or Bagian = EBagian.Langganan_Perwakilan Or Bagian = EBagian.Supermarket_Perwakilan Then
            If GridView1.FocusedColumn.FieldName = "Koli" Or GridView1.FocusedColumn.FieldName = "Kuantum" Or GridView1.FocusedColumn.FieldName = "Pieces" Then
                ...
                If (stok - stuffing) < col("Pieces") Then
                    GridView1.CloseEditor()
                    MsgBox("Stok Untuk Barang Ini Tidak Mencukupi, Segera Lakukan Order Ke Pabrik !" & vbCrLf &
                           "Sisa Stok : " & stok.ToString() & " Pieces" & vbCrLf &
    ...
    Private Function CekStok(Kode As String, Gudang As String) As Integer
        If Gudang = "" Then
            ...
            Return 0
        Else
            Using dt = SelectCon.execute("SELECT STOK_PCS FROM V_STOK_BERJALAN WITH (NOLOCK) WHERE GUDANG='" & Gudang & "' AND ID_BARANG='" & Kode & "'")
    ...
    Private Function CekStokStuffing(Kode As String, Gudang As String) As Integer
        If Gudang = "" Then
            ...
            Return 0
        Else
            Using dt = SelectCon.execute("SELECT PCS-PCS_T PCS FROM STUFFING A WITH (NOLOCK) JOIN V_D_STUFF_T_NOTA B WITH (NOLOCK) ON A.ID_TRANSAKSI=B.ID_TRANSAKSI WHERE ST=0 AND GUDANG='" & Gudang & "' AND ID_BARANG='" & Kode & "'")
                If dt.Rows.Count > 0 Then
                    Return dt.Rows(0).Item(0)
                End If
            End Using
    ...
    Private Sub RDPembayaran_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles RDPembayaran.SelectedIndexChanged
        If RDPembayaran.SelectedIndex = 1 Then
            SetData("SELECT HARI FROM CUSTOMER WITH (NOLOCK) WHERE ID='" & TxtIDCustomer.Text & "'", {TxtHari})
    ...
    Function CurrentPromo() As String
        ...
        Using p As DataTable = SelectCon.execute("SELECT B.NAMA_PROMO from LINK_BARANG_PROMO A WITH (NOLOCK) JOIN PROMO B WITH (NOLOCK) ON A.KODE_PROMO=B.KODE WHERE A.ID_BARANG='" & dr("ID Barang") & "' AND '" & Format(TglPengakuan.DateTime, "yyyy-MM-dd") & "' BETWEEN TGL_AWAL AND TGL_AKHIR")
        ...
        If GridView1.FocusedColumn.FieldName = "Kode Barang" Then
            ...
            Using dt_cari = SelectCon.execute("SELECT TOP 1 A.ID,A.KODE,A.NAMA," & IIf(EnumDescription(Bagian).Contains("Langganan"), "A.SAT_DIST1 AS SATUAN ,ISNULL(C.HARGA_BARU,A.HARGA_DIST) AS HARGA, A.QTY_KOLI AS ISI ", "A.SAT_SUPER1 AS SATUAN ,ISNULL(C.HARGA_BARU,A.HARGA_SUPER) AS HARGA, A.QTY_DIST*A.QTY_KOLI AS ISI ") & " ," & IIf(EnumDescription(Bagian).Contains("Langganan"), "A.QTY_DIST", "1") & " AS KONV,C.TGL_AWAL TGL_PRICE FROM BARANG A WITH (NOLOCK) LEFT JOIN VI_PRICE_LIST_PROMO C WITH (NOLOCK) ON A.ID=C.ID_BARANG " & "AND (C.ID='" & TxtIDCustomer.Text & "' OR C.ID='UMUM') AND CONVERT(DATE,'" & Format(IIf(TglTransaksi.DateTime > TglPengakuan.DateTime, DateAdd(DateInterval.Day, 1, TglPengakuan.DateTime), TglPengakuan.DateTime), "MM-dd-yyyy") & "') BETWEEN C.TGL_AWAL AND C.TGL_AKHIR " & IIf(EnumDescription(Bagian).Contains("Langganan"), " AND C.JENIS='Langganan'", " AND C.JENIS='Supermarket'") & " LEFT JOIN LINK_BARANG_DIVISI D WITH (NOLOCK) ON A.ID=D.ID_BARANG WHERE (A.ID='" & col("Kode Barang") & "' OR A.KODE='" & col("Kode Barang") & "') " & " AND A.STATUS_AKTIF=1 AND D.KODE_DIVISI='" & TxtDivisi.Text & "' ORDER BY C.TGL_AWAL DESC,C.ID")
                If dt_cari.Rows.Count = 1 Then
                    ...
                    col("Harga Satuan") = IIf(RDJenisPPN.EditValue = "Exclude", dt_cari.Rows(0).Item("HARGA") / 1.1, dt_cari.Rows(0).Item("HARGA"))
                    col("Disc %") = 0
                    col("Disc Satuan") = 0
                    col("Subtotal") = 0
...
CariData:
            ...
            Else
                ...
                End If
            End If
            Using dt_cari = SelectCon.execute("SELECT TOP 1 A.ID,A.KODE,A.NAMA," & IIf(EnumDescription(Bagian).Contains("Langganan"), "A.SAT_DIST1 AS SATUAN ,ISNULL(C.HARGA_BARU,A.HARGA_DIST) AS HARGA, A.QTY_KOLI AS ISI ", "A.SAT_SUPER1 AS SATUAN ,ISNULL(C.HARGA_BARU,A.HARGA_SUPER) AS HARGA, A.QTY_DIST*A.QTY_KOLI AS ISI ") & " ," & IIf(EnumDescription(Bagian).Contains("Langganan"), "A.QTY_DIST", "1") & " AS KONV,C.TGL_AWAL TGL_PRICE FROM BARANG A LEFT JOIN VI_PRICE_LIST_PROMO C ON A.ID=C.ID_BARANG " & "AND (C.ID='" & TxtIDCustomer.Text & "' OR C.ID='UMUM') AND CONVERT(DATE,'" & Format(IIf(TglTransaksi.DateTime > TglPengakuan.DateTime, DateAdd(DateInterval.Day, 1, TglPengakuan.DateTime), TglPengakuan.DateTime), "MM-dd-yyyy") & "') BETWEEN C.TGL_AWAL AND C.TGL_AKHIR " & IIf(EnumDescription(Bagian).Contains("Langganan"), " AND C.JENIS='Langganan'", " AND C.JENIS='Supermarket'") & " LEFT JOIN LINK_BARANG_DIVISI D ON A.ID=D.ID_BARANG WHERE A.ID='" & col("ID Barang") & "' " & " AND A.STATUS_AKTIF=1 AND D.KODE_DIVISI='" & TxtDivisi.Text & "' ORDER BY C.TGL_AWAL DESC,C.ID")
    ...
    Private Sub RDJenisPPN_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles RDJenisPPN.SelectedIndexChanged
        For Each dr As DataRow In dt.Rows
            If RDJenisPPN.EditValue = "Exclude" Then
                dr.Item("Harga Satuan") = dr.Item("Harga Satuan") / 1.1
            Else
                dr.Item("Harga Satuan") = dr.Item("Harga Satuan") * 1.1
            End If
        Next
    ...
    Private Sub TxtIDPO_EditValueChanged(sender As Object, e As System.EventArgs) Handles TxtIDPO.EditValueChanged
        If Status_Edit = False Then
            ...
            LoadData.GetData("SELECT A.URUTAN,A.ID_BARANG,B.KODE,B.NAMA,A.SATUAN,A.ISI,A.KOLI-A.KOLI_T,A.QUANTITY-A.QUANTITY_T,A.PCS-A.PCS_T,A.KOLI-A.KOLI_T,A.QUANTITY-A.QUANTITY_T,A.PCS-A.PCS_T,A.KONVERSI,A.HARGA,A.DISC,A.DISKON_SATUAN,ROUND(A.QUANTITY * (A.HARGA - A.DISKON_SATUAN),2) AS SUBTOTAL FROM V_D_PO_T_DO A WITH (NOLOCK) LEFT JOIN BARANG B WITH (NOLOCK) ON A.ID_BARANG=B.ID WHERE A.ID_TRANSAKSI='" & TxtIDPO.Text & "' AND A.ST=0 ORDER BY URUTAN")
            LoadData.SetDataDetail(dt, False)
            If TxtIDPO.Text <> "" Then _
                SetData("SELECT TGL_PRICE FROM VI_PRICE_LIST WITH (NOLOCK) WHERE ID_BARANG='" & GridView1.GetDataRow(0)("ID Barang") & "' AND HARGA_BARU=" & GridView1.GetDataRow(0)("Harga Satuan") & "", {TglPricelist})
            Urutan()
            HitungTanpaDiskon()
        Else
            Using MYLoadData As New _LoadData
                MYLoadData.GetData("SELECT [NO_PO] FROM PURCHASE_ORDER WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDPO.Text & "'")
    ...
    Private Sub BtnRubahHarga_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BtnRubahHarga.ItemClick
        Using LoadDataToGrid As New _LoadDataToGrid
            ...
            'Else
            LoadDataToGrid.BeginInit("SELECT 'Pilih' AS PILIH,HARGA_BARU,TGL_AWAL TGL_PRICE FROM VI_PRICE_LIST_PROMO WITH (NOLOCK) WHERE ID_BARANG='" & GridView1.GetFocusedDataRow()("ID Barang") & "' AND JENIS='" & IIf(EnumDescription(Bagian).Contains("Supermarket"), "Supermarket", "Langganan") & "' AND (ID='UMUM' OR ID='" & TxtIDCustomer.Text & "') ORDER BY TGL_AWAL DESC")
            'End If
    ...
    Private Sub TxtKodeCabang_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtKodeCabang.EditValueChanged
        'SetData("SELECT NAMA_CABANG FROM DETAIL_CUSTOMER_CABANG WITH (NOLOCK) WHERE ID='" & TxtIDCustomer.Text & "' AND KODE_CABANG='" & TxtKodeCabang.Text & "'", {TxtNamaCabang})
        SetData("SELECT NAMA_CABANG,ALAMAT_CABANG FROM DETAIL_CUSTOMER_CABANG WITH (NOLOCK) WHERE ID='" & TxtIDCustomer.Text & "' AND KODE_CABANG='" & TxtKodeCabang.Text & "'", {TxtNamaCabang, TxtAlamatKirim})
    End Sub
...
#Region "Declare"
    ...
    Protected Sub HitungTanpaDiskon()
        ...
        For Each col As DataRow In dt.Rows
            ...
            End If
            TxtSubTotal.Text = Math.Round(Total, 2)
        Next
        ...
        DPP = Math.Round(Total - CDbl(TempTotalDiskon), 2)
...
DiskonQty2:             TempTotalDiskon = CDbl(TxtDiskonQty1Nominal.Value) + CDbl(TxtDiskonRegulerNominal.Value) + CDbl(TxtDiskon1Nominal.Value) +
        ...
        DPP = Math.Round(Total - TempTotalDiskon, 2)
...
#End Region
    ...
    Private Function CekCustomerSBU(id As String, divisi As String) As Boolean
        ...
        If SelectCon.execute("SELECT * FROM (SELECT DC.LIMIT, KODE_DIVISI, C.ID FROM CORPORATION CR WITH (NOLOCK) JOIN LINK_CORPORATION_CUSTOMER COR WITH (NOLOCK) ON CR.KODE=COR.KODE_CORPORATION JOIN CUSTOMER C WITH (NOLOCK) ON COR.ID_CUSTOMER=C.ID JOIN DETAIL_CORPORATION_LIMITPIUTANG DC ON COR.KODE_CORPORATION=DC.KODE JOIN LINK_SBU_DIVISI D ON DC.KODE_SBU=D.KODE_SBU WHERE CR.STATUS_AKTIF=1 AND C.GROUP_CUSTOMER<>'Supermarket' AND C.SYARAT_PEMBAYARAN_KREDIT=1 UNION SELECT LIMIT, KODE_DIVISI, DC.ID FROM CUSTOMER C WITH (NOLOCK) JOIN DETAIL_CUSTOMER_LIMITPIUTANG DC WITH (NOLOCK) ON C.ID=DC.ID JOIN LINK_SBU_DIVISI D ON DC.KODE_SBU=D.KODE_SBU WHERE C.GROUP_CUSTOMER<>'Supermarket' AND C.SYARAT_PEMBAYARAN_KREDIT=1) A WHERE LIMIT > 0 AND ID='" & id & "' AND KODE_DIVISI='" & divisi & "'").Rows.Count > 0 Then
            Return True
        End If
    ...
    Private Function CekCustomerKontan(id As String) As Boolean
        If SelectCon.execute("SELECT SYARAT_PEMBAYARAN_KONTAN FROM CUSTOMER WITH (NOLOCK) WHERE ID='" & id & "'").Rows.Item(0).Item(0) Then
            Return True
        End If
        Return False
    End Function
    ...
    Private Sub TxtIDCustomer_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtIDCustomer.EditValueChanged
        ...
        AddRow(dt)
        RDPembayaran.SelectedIndex = -1
        Using dtCustomer = SelectCon.execute("SELECT KODE_APPROVE,NAMA,ALAMAT_KANTOR,LOKASI_PENGIRIMAN,SYARAT_PEMBAYARAN_KREDIT,NO_NPWP FROM CUSTOMER  WITH (NOLOCK) WHERE ID='" & TxtIDCustomer.Text & "'")
    ...
    Private Sub TxtIdSalesman_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtIdSalesman.EditValueChanged
        SetData("SELECT NAMA_USER FROM USERS WITH (NOLOCK) WHERE ID_USER ='" & TxtIdSalesman.Text & "'", {TxtNamaSalesman})
    End Sub
    ...
    Private Sub txtGudangCustomer_ButtonClick(sender As Object, e As ButtonPressedEventArgs) Handles txtGudangCustomer.ButtonClick
        IDCustomer = TxtIDCustomer.Text
        txtGudangCustomer.Text = Search(FrmPencarian.KeyPencarian.GudangCustomer, , , , , "Gudang Customer")
    End Sub

    Private Sub txtGudangCustomer_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtGudangCustomer.KeyPress
        IDCustomer = TxtIDCustomer.Text
        If CharKeypress(txtGudangCustomer, e) Then txtGudangCustomer.Text = Search(FrmPencarian.KeyPencarian.GudangCustomer, , , , , "Gudang Customer")
    End Sub

    Private Sub txtGudangCustomer_EditValueChanged(sender As Object, e As EventArgs) Handles txtGudangCustomer.EditValueChanged
        SetData("SELECT NAMA_GUDANG FROM GUDANG_CUSTOMER WITH (NOLOCK) WHERE KODE_GUDANG_CUSTOMER='" & txtGudangCustomer.Text & "' AND KODE_CUSTOMER='" & TxtIDCustomer.Text & "'", {txtNamaGudangCustomer})
        If TxtAlamatKirim.Text = "" Then
            SetData("SELECT ALAMAT_KIRIM FROM DELIVERY_ORDER WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDTransaksi.Text & "' AND KODE_CUSTOMER='" & TxtIDCustomer.Text & "'", {TxtAlamatKirim})
        End If
    End Sub
End Class
