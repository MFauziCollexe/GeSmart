...
Public MustInherit Class FrmNotaSJ
    ...
    Private TempNamaNota As String = Nothing
    ...
    Property Bagian As EBagian
        Set(value As EBagian)
            ...
            If value.ToString.Contains("Perwakilan") Then
                ...
                LyTglDSRPusat.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Never
            ElseIf value.ToString.Contains("Pusat") Then
                ...
                TxtNoTransaksi.Properties.Buttons(0).Visible = False
                LyTglDSRPusat.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Always
            End If
...
#Region "Shared Sub"
    ...
    Private Sub Cetak() Handles _Toolbar1_Button6.ItemClick
        ...
        If SelectCon.execute("SELECT SJ_TANPA_HARGA FROM NOTA WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'")(0)(0) Then
...
#Region "Input"
    ...
    Protected Sub GenerateAuto()
        If Not Status_Edit Then
            If Bagian = EBagian.Langganan_Perwakilan Or Bagian = EBagian.Lain_Perwakilan Or Bagian = EBagian.Supermarket_Perwakilan Then
                ...
                Using dtGenerate = SelectCon.execute("SELECT NO_NOTA FROM NOTA WITH (NOLOCK) WHERE NO_NOTA LIKE '" & TxtNamaDivisi.Text & MyFormat & "%' AND YEAR(TGL_PENGAKUAN)=" & Format(TglPengakuan.EditValue, "yyyy") & " AND DIVISI='" & TxtDivisi.Text & "' ORDER BY NO_NOTA DESC")
    ...
    Protected Function Generate() As Boolean
        If Status_Edit = False Then
            ...
            Using dtGenerate = SelectCon.execute("SELECT NO_NOTA FROM NOTA WITH (NOLOCK) WHERE NO_NOTA = '" & TxtNoTransaksi.Text & "' AND YEAR(TGL_PENGAKUAN)=" & Format(TglPengakuan.EditValue, "yyyy") & " AND DIVISI='" & TxtDivisi.Text & "'")
                If dtGenerate.Rows.Count > 0 Then
    ...
    Protected Sub Simpan()
        ...
        If EnumDescription(Bagian).Contains("Perwakilan") Then
            ...
            If TglPengakuan.DateTime < TglTransaksi.DateTime Then
                MsgBox("Tanggal pengakuan tidak boleh lebih kecil dari tanggal transaksi!!!", MsgBoxStyle.Information, "Peringatan")
                Exit Sub
            End If
        End If
        ...
        If SelectCon.execute("SELECT PEMBAYARAN FROM DELIVERY_ORDER WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDDO.Text & "' AND PEMBAYARAN='Kredit'").Rows.Count > 0 Then
                ...
                If SelectCon.execute("SELECT LIMIT_PIUTANG FROM PERUSAHAAN")(0)(0) Then
                    Using dt = SelectCon.execute("SELECT LIMIT, PIUTANG, SISA FROM [F_LIMIT_PIUTANG]('" & TxtIDCustomer.Text & "', '" & TxtDivisi.Text & "')")
                        If (dt.Rows(0).Item("SISA")) < TxtTotal.Value Then
                            MsgBox("Limit Piutang Terlampaui !" & vbCrLf &
                                   "Limit : Rp " & Format(dt.Rows(0).Item("LIMIT"), "n0") & vbCrLf &
...
#Region "Edit"
    Protected Sub GetData()
        If TxtIDTransaksi.Text <> "" Then
            Using MyLoadData As New _LoadData
                ...
                If EnumDescription(Bagian).Contains("Perwakilan") Then
                    Str_MyQuery = "SELECT A.URUTAN,A.ID_BARANG,C.KODE,C.NAMA,A.SATUAN,(B.KOLI-B.KOLI_T)+A.KOLI,(B.QUANTITY-B.QUANTITY_T)+A.QUANTITY,(B.PCS-B.PCS_T)+A.PCS,A.KOLI,A.QUANTITY,A.PCS,A.HARGA,A.DISC,A.DISKON_SATUAN,(A.PCS * ((A.HARGA-A.DISKON_SATUAN)/A.KONVERSI)),A.KONVERSI,A.ISI FROM (NOTA AA INNER JOIN DETAIL_NOTA A ON AA.ID_TRANSAKSI=A.ID_TRANSAKSI) LEFT JOIN V_D_STUFF_T_NOTA B WITH (NOLOCK) ON AA.ID_STUFFING=B.ID_TRANSAKSI AND A.ID_BARANG=B.ID_BARANG LEFT JOIN BARANG C WITH (NOLOCK) ON A.ID_BARANG=C.ID WHERE A.ID_TRANSAKSI='" & TxtIDTransaksi.Text & "' ORDER BY A.URUTAN"
                ElseIf EnumDescription(Bagian).Contains("Pusat") Then
                    Str_MyQuery = "SELECT A.URUTAN,A.ID_BARANG,C.KODE,C.NAMA,A.SATUAN,(B.KOLI-B.KOLI_T)+A.KOLI,(B.QUANTITY-B.QUANTITY_T)+A.QUANTITY,(B.PCS-B.PCS_T)+A.PCS,A.KOLI,A.QUANTITY,A.PCS,A.HARGA,A.DISC,A.DISKON_SATUAN,(A.PCS * ((A.HARGA-A.DISKON_SATUAN)/A.KONVERSI)),A.KONVERSI,A.ISI FROM (NOTA AA INNER JOIN DETAIL_NOTA A ON AA.ID_TRANSAKSI=A.ID_TRANSAKSI) LEFT JOIN V_D_DB_PUSAT_T_NOTA B WITH (NOLOCK) ON AA.ID_DO=B.ID_TRANSAKSI AND A.ID_BARANG=B.ID_BARANG LEFT JOIN BARANG C WITH (NOLOCK) ON A.ID_BARANG=C.ID WHERE A.ID_TRANSAKSI='" & TxtIDTransaksi.Text & "' ORDER BY A.URUTAN"
                End If
    ...
    Protected Sub SimpanEdit()
        ...
        GridView1.CloseEditor()
        If TxtNoSJPB.Text <> "" Then
            Using dtGenerate = SelectCon.execute("SELECT NO_SJPB FROM NOTA WITH (NOLOCK) WHERE NO_SJPB = '" & TxtNoSJPB.Text & "' AND YEAR(TGL_PENGAKUAN)=" & Format(TglPengakuan.EditValue, "yyyy") & " AND ID_TRANSAKSI<>'" & TxtIDTransaksi.Text & "'")
                If dtGenerate.Rows.Count > 0 Then
                    MsgBox("Nomor SJPB " & dtGenerate.Rows(0).Item(0) & " Telah dipakai !")
                    Exit Sub
                End If
...
#End Region
    ...
    Private Sub FrmDeliveryOrder_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        TglTransaksi.DateTime = Format(Now.Date, "dd/MM/yyyy")
        TglDSRPusat.DateTime = Format(Now.Date, "dd/MM/yyyy")
        InitGrid.Clear()
    ...
    Private Sub TxtIDCustomer_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtIDCustomer.EditValueChanged
        SetData("SELECT KODE_APPROVE,NAMA,ALAMAT_KANTOR FROM CUSTOMER WITH(NOLOCK) WHERE ID='" & TxtIDCustomer.Text & "'", {TxtKodeApprove, TxtNama, TxtAlamatKantor})
    End Sub
    ...
    Private Sub GridView1_KeyUp(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles GridView1.KeyUp
        If e.KeyCode = 46 Then
            If Bagian = EBagian.Lain_Pusat Or Bagian = EBagian.Langganan_Pusat Or Bagian = EBagian.Supermarket_Pusat Then
...
CariData:
    ...
    Private Sub TxtNoStuffing_ButtonClick(sender As Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles TxtNoStuffing.ButtonClick
        If Not Empty({TglPengakuan}) Then
    ...
    Private Sub TxtIDStuffing_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtIDStuffing.EditValueChanged
        ...
        SetData("SELECT A.ID_DO, A.NO_STUFFING, B.CABANG, B.ALAMAT_KIRIM, A.GUDANG, B.KETERANGAN_CETAK, B.KETERANGAN_INTERNAL FROM STUFFING A WITH (NOLOCK) JOIN DELIVERY_ORDER B WITH (NOLOCK) ON A.ID_DO=B.ID_TRANSAKSI WHERE A.ID_TRANSAKSI='" & TxtIDStuffing.Text & "'", {TxtIDDO, TxtNoStuffing, TxtKodeCabang, TxtAlamatKirim, TxtKodeGudang, txtKeterangan, txtKeteranganInternal})
        TxtKodeGudang.Enabled = False
        If Status_Edit = False Then
            Using MyLoadData As New _LoadData
                MyLoadData.GetData("SELECT NO_DO,TGL_PENGAKUAN,DIVISI,JENIS_PPN,KODE_CUSTOMER,KODE_APPROVE,[DISC_QTY],[DISC_REG],[DISC_1],[DISC_2],[DISC_3],[CASH_DISC],[DISC_QTY1],[BEBAS_PPN], SUBTOTAL FROM DELIVERY_ORDER WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDDO.Text & "'")
                ...
                If Math.Abs(TxtSubTotal.Value - SUBTOTAL_DB) < 1000 Then
                    MyLoadData.GetData("SELECT DISC_QTY_NOMINAL, DISC_REG_NOMINAL, DISC_1_NOMINAL, DISC_2_NOMINAL, DISC_3_NOMINAL, CASH_DISC_NOMINAL, DISC_QTY_NOMINAL1, DPP, PPN FROM DELIVERY_ORDER WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDDO.Text & "'")
        ...
        If Status_Edit = False Then
            If EnumDescription(Bagian).Contains("Pusat") Then
                Using MyLoadData As New _LoadData
                    ...
                    MyLoadData.GetData("SELECT A.URUTAN,A.ID_BARANG,B.KODE,B.NAMA,A.SATUAN,A.KOLI-A.KOLI_T,A.QUANTITY-A.QUANTITY_T,A.PCS-A.PCS_T,0,0,0,A.HARGA,A.DISC,A.DISKON_SATUAN,(A.PCS * ((A.HARGA-A.DISKON_SATUAN)/A.KONVERSI)),A.KONVERSI,A.ISI FROM V_D_DB_PUSAT_T_NOTA A WITH (NOLOCK) LEFT JOIN BARANG B WITH (NOLOCK) ON A.ID_BARANG=B.ID WHERE A.ID_TRANSAKSI='" & TxtIDDO.Text & "' AND A.ST=0 ORDER BY URUTAN")
                    MyLoadData.SetDataDetail(dt, False)
                    Urutan()
                    Hitung()
            ...
            Using MyLoadData As New _LoadData
                MyLoadData.GetData("SELECT PRINT_PKP FROM DELIVERY_ORDER WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDDO.Text & "'")
    ...
    Private Sub TxtKodeEkspedisi_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtKodeEkspedisi.EditValueChanged
        SetData("SELECT NAMA FROM EKSPEDISI WITH(NOLOCK) WHERE KODE='" & TxtKodeEkspedisi.Text & "'", {TxtNamaEkspedisi})
    End Sub
...
#End Region
    ...
    Sub generateSJPB()
        If ChkSJTanpaHarga.Checked Then
            Dim urut As Short
            Using dtGenerate = SelectCon.execute("SELECT ISNULL(MAX(CAST(NO_SJPB AS BIGINT)), 0) NO_SJPB FROM NOTA WITH (NOLOCK) WHERE YEAR(TGL_PENGAKUAN)=" & Format(TglPengakuan.EditValue, "yyyy") & " AND BAGIAN LIKE '%Supermarket%'")
                If dtGenerate.Rows.Count = 0 Then
                    urut = 1
                Else
                    urut = Microsoft.VisualBasic.Right(dtGenerate.Rows(0).Item(0), 6) + 1
