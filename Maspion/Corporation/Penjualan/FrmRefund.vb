...
#Region "Edit"
    ...
    Protected Sub Hapus()
        ...
        Using SQLServer As New SQLServerTransaction
            ...
            HitungAwal()
            If SQLServer.UpdateHeader("[SUBTOTAL] ,[DISC_QTY] ,[DISC_QTY_NOMINAL] ,[DISC_REG] ,[DISC_REG_NOMINAL] ,[DISC_1] ,[DISC_1_NOMINAL] ,[DISC_2] ,[DISC_2_NOMINAL] ,[DISC_3] ,[DISC_3_NOMINAL] ,[CASH_DISC] ,[CASH_DISC_NOMINAL] ,[DISC_QTY1] ,[DISC_QTY_NOMINAL1] ,[DPP] ,[PPN]", {subtotalawal, diskonqty1awal, diskonqty1nominalawal, diskonregulerawal, diskonregulernominalawal, diskon1awal, diskon1nominalawal, diskon2awal, diskon2nominalawal, diskon3awal, diskon3nominalawal, cashdiskonawal, cashdiskonnominalawal, diskonqty2awal, diskonqty2nominalawal, dppawal, ppnawal}, "NOTA", "ID_TRANSAKSI='" & TxtIDNota.Text & "'") = False Then Exit Sub
            SQLServer.EndTransaction()
...
#End Region
    ...
    Private Sub TxtKode_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtIDCustomer.EditValueChanged
        SetData("SELECT NAMA,ALAMAT_KANTOR FROM CUSTOMER WITH (NOLOCK) WHERE ID='" & TxtIDCustomer.Text & "'", {TxtNama, TxtAlamatKantor})
    End Sub
    ...
    End Sub
    Dim subtotalawal As Double = 0
    Dim diskonqty1awal As Double = 0
    Dim diskonqty1nominalawal As Double = 0
    Dim diskonregulerawal As Double = 0
    Dim diskonregulernominalawal As Double = 0
    Dim diskon1awal As Double = 0
    Dim diskon1nominalawal As Double = 0
    Dim diskon2awal As Double = 0
    Dim diskon2nominalawal As Double = 0
    Dim diskon3awal As Double = 0
    Dim diskon3nominalawal As Double = 0
    Dim cashdiskonawal As Double = 0
    Dim cashdiskonnominalawal As Double = 0
    Dim diskonqty2awal As Double = 0
    Dim diskonqty2nominalawal As Double = 0
    Dim dppawal As Double = 0
    Dim ppnawal As Double = 0
    Protected Sub HitungAwal()
        On Error Resume Next
        Dim TempTotalDiskon As Double = 0
        Dim Total As Double = 0
        Dim Subtotal As Decimal = 0
        For Each col As DataRow In dt.Rows
            ...
            If Subtotal >= 0 Then
                Total = Total + Subtotal
            End If
            subtotalawal = Math.Round(Total)
        Next
        diskonqty1awal = TxtDiskonQty1.Value
        diskonqty2awal = TxtDiskonQty2.Value
        diskonregulerawal = TxtDiskonReguler.Value
        diskon1awal = TxtDiskon1.Value
        diskon2awal = TxtDiskon2.Value
        diskon3awal = TxtDiskon3.Value
        cashdiskonawal = TxtCashDiskon.Value
DiskonQty1: diskonqty1nominalawal = CDbl(dt.Compute("Sum([Qty Nota])", "")) * CDbl(diskonqty1awal)

        diskonqty1awal = CDbl(diskonqty1nominalawal) / CDbl(dt.Compute("Sum([Qty Nota])", ""))

DiskonReguler: TempTotalDiskon = CDbl(diskonqty1nominalawal)
        diskonregulernominalawal = Math.Round((Total - TempTotalDiskon) * CDbl(diskonregulerawal) / 100)

        TempTotalDiskon = CDbl(diskonqty1nominalawal)
        diskonregulerawal = Math.Round(CDbl(diskonregulernominalawal) / (Total - TempTotalDiskon) * 100)

Diskon1: TempTotalDiskon = CDbl(diskonqty1nominalawal) + CDbl(diskonregulernominalawal)
        diskon1nominalawal = Math.Round((Total - TempTotalDiskon) * CDbl(diskon1awal) / 100)

        TempTotalDiskon = CDbl(diskonqty1nominalawal) + CDbl(diskonregulernominalawal)
        diskon1awal = Math.Round(CDbl(diskon1nominalawal) / (Total - TempTotalDiskon) * 100)

Diskon2: TempTotalDiskon = CDbl(diskonqty1nominalawal) + CDbl(diskonregulernominalawal) + CDbl(diskon1nominalawal)
        diskon2nominalawal = Math.Round((Total - TempTotalDiskon) * CDbl(diskon2awal) / 100)

        TempTotalDiskon = CDbl(diskonqty1nominalawal) + CDbl(diskonregulernominalawal) + CDbl(diskon1nominalawal)
        diskon2awal = Math.Round(CDbl(diskon2nominalawal) / (Total - TempTotalDiskon) * 100)

Diskon3: TempTotalDiskon = CDbl(diskonqty1nominalawal) + CDbl(diskonregulernominalawal) + CDbl(diskon1nominalawal) +
                        CDbl(diskon2nominalawal)
        diskon3nominalawal = Math.Round((Total - TempTotalDiskon) * CDbl(diskon3awal) / 100)

        TempTotalDiskon = CDbl(diskonqty1nominalawal) + CDbl(diskonregulernominalawal) + CDbl(diskon1nominalawal) +
                        CDbl(diskon2nominalawal)
        diskon3awal = Math.Round(CDbl(diskon3nominalawal) / (Total - TempTotalDiskon) * 100)

CashDiskon: TempTotalDiskon = CDbl(diskonqty1nominalawal) + CDbl(diskonregulernominalawal) + CDbl(diskon1nominalawal) +
                        CDbl(diskon2nominalawal) + CDbl(diskon3nominalawal)
        cashdiskonnominalawal = Math.Round((Total - TempTotalDiskon) * CDbl(cashdiskonawal) / 100)

        TempTotalDiskon = CDbl(diskonqty1nominalawal) + CDbl(diskonregulernominalawal) + CDbl(diskon1nominalawal) +
                        CDbl(diskon2nominalawal) + CDbl(diskon3nominalawal)
        cashdiskonawal = Math.Round(CDbl(cashdiskonnominalawal) / (Total - TempTotalDiskon) * 100)

DiskonQty2: TempTotalDiskon = CDbl(diskonqty1nominalawal) + CDbl(diskonregulernominalawal) + CDbl(diskon1nominalawal) +
                        CDbl(diskon2nominalawal) + CDbl(diskon3nominalawal) + CDbl(cashdiskonnominalawal)
        diskonqty2nominalawal = Math.Round((Total - TempTotalDiskon) * CDbl(diskonqty2awal) / 100)
        TempTotalDiskon = CDbl(diskonqty1nominalawal) + CDbl(diskonregulernominalawal) + CDbl(diskon1nominalawal) +
                        CDbl(diskon2nominalawal) + CDbl(diskon3nominalawal) + CDbl(cashdiskonnominalawal)
        diskonqty2awal = Math.Round(CDbl(diskonqty2nominalawal) / (Total - TempTotalDiskon) * 100)

        TempTotalDiskon = CDbl(diskonqty1nominalawal) + CDbl(diskonregulernominalawal) + CDbl(diskon1nominalawal) +
                        CDbl(diskon2nominalawal) + CDbl(diskon3nominalawal) + CDbl(cashdiskonnominalawal) +
            CDbl(diskonqty2nominalawal)
