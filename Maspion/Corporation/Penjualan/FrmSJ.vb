...
#Region "Input"
    ...
    Protected Function Generate() As Boolean
        Dim urut As Short
        Using dtGenerate = SelectCon.execute("SELECT ISNULL(MAX(CAST(NO_SJ AS BIGINT)), 0) NO_SJ FROM SURAT_JALAN WITH (NOLOCK) WHERE YEAR(TGL_PENGAKUAN)=" & Format(TglPengakuan.EditValue, "yyyy") & " AND BAGIAN LIKE '%Langganan%'")
        ...
        Using dtGenerate = SelectCon.execute("SELECT ISNULL(MAX(CAST(REPLACE(ID_TRANSAKSI,'SJ','') AS BIGINT)),0) AS ID FROM SURAT_JALAN WITH (NOLOCK)")
    ...
    Private Sub TxtIDNota_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtIDNota.EditValueChanged
        If Status_Edit = False Then
            Using MyLoadData As New _LoadData
                LoadData.GetData("SELECT NO_NOTA,TGL,DIVISI,KODE_CUSTOMER,KODE_APPROVE,ALAMAT_KIRIM,KETERANGAN_CETAK,KETERANGAN_INTERNAL FROM NOTA WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDNota.Text & "'")
