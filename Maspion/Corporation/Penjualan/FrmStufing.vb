...
#Region "Input"
    ...
    Protected Sub Generate()
        Dim urut As Short
        Using dtGenerate = SelectCon.execute("SELECT TOP 1 NO_STUFFING FROM STUFFING WHERE NO_STUFFING Like 'ST/" & TxtNamaDivisi.Text & "%' AND YEAR(TGL_PENGAKUAN)=" & Format(TglPengakuan.EditValue, "yyyy") & " AND DIVISI='" & TxtDivisi.Text & "' ORDER BY NO_STUFFING DESC")
...
#End Region
    ...
    Private Sub TxtKode_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtIDCustomer.EditValueChanged
        SetData("SELECT KODE_APPROVE,NAMA,ALAMAT_KANTOR,LOKASI_PENGIRIMAN,SYARAT_PEMBAYARAN_KREDIT FROM CUSTOMER WITH (NOLOCK) WHERE ID='" & TxtIDCustomer.Text & "'", {TxtKodeApprove, TxtNama, TxtAlamatKantor})
    End Sub
    ...
    Private Sub TxtIDNota_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtIDDO.EditValueChanged
        ...
        LblGudangDO.Text = "Gudang DO : " & SelectCon.execute("SELECT NAMA_GUDANG FROM DELIVERY_ORDER DO WITH (NOLOCK) JOIN GUDANG G WITH (NOLOCK) ON DO.GUDANG=G.KODE WHERE DO.ID_TRANSAKSI='" & TxtIDDO.Text & "'")(0)(0)
    End Sub
