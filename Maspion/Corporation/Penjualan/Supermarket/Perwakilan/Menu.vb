Public Class MenuDeliveryOrderSupermarketPerwakilan
    ...
    Private Sub GetData()
        'TBMenu.DataSource = SelectCon.execute("SET NOCOUNT ON SELECT DISTINCT A.ID_TRANSAKSI,A.NO_DO,A.TGL,A.TGL_PENGAKUAN,B.NAMA,D.NAMA_USER,G.NAMA_GUDANG,A.PEMBAYARAN,A.TERMS,DPP+PPN AS TOTAL,A.BATAL,ST.STK FROM DELIVERY_ORDER A WITH (NOLOCK) JOIN V_D_DB_PERW_T_STUFF ST WITH (NOLOCK) ON A.ID_TRANSAKSI=ST.ID_TRANSAKSI LEFT JOIN CUSTOMER B WITH (NOLOCK) ON A.KODE_CUSTOMER=B.ID LEFT JOIN LINK_USER_DIVISI C WITH (NOLOCK) ON A.DIVISI=C.KODE_DIVISI LEFT JOIN USERS D WITH (NOLOCK) ON A.CRUSER=D.ID_USER LEFT JOIN GUDANG G WITH (NOLOCK) ON A.GUDANG=G.KODE WHERE BAGIAN='Supermarket Perwakilan' AND JENIS_DO='Ada Barang' AND PERIODE='" & periode & "' AND C.ID_USER='" & UserID & "'")
        'TBMenu.DataSource = SelectCon.execute("SET NOCOUNT ON SELECT DISTINCT A.ID_TRANSAKSI,A.NO_DO,A.TGL,A.TGL_PENGAKUAN,B.NAMA,D.NAMA_USER,G.NAMA_GUDANG,A.PEMBAYARAN,A.TERMS,DPP+PPN AS TOTAL,A.BATAL,ST.STK FROM DELIVERY_ORDER A WITH (NOLOCK) JOIN V_D_DB_PERW_T_STUFF ST WITH (NOLOCK) ON A.ID_TRANSAKSI=ST.ID_TRANSAKSI INNER JOIN CUSTOMER B WITH (NOLOCK) ON A.KODE_CUSTOMER=B.ID INNER JOIN LINK_USER_DIVISI C WITH (NOLOCK) ON A.DIVISI=C.KODE_DIVISI INNER JOIN USERS D WITH (NOLOCK) ON A.CRUSER=D.ID_USER INNER JOIN GUDANG G WITH (NOLOCK) ON A.GUDANG=G.KODE WHERE BAGIAN='Supermarket Perwakilan' AND JENIS_DO='Ada Barang' AND PERIODE='" & periode & "' AND C.ID_USER='" & UserID & "'")
        ...
        On a.ID_TRANSAKSI=st.ID_TRANSAKSI) join LINK_USER_DIVISI lud WITH (NOLOCK) on a.DIVISI=lud.KODE_DIVISI where a.BAGIAN='Supermarket Perwakilan' AND a.JENIS_DO='Ada Barang' AND a.PERIODE='" & periode & "' and lud.ID_USER='" & UserID & "' order by a.TGL,a.ID_TRANSAKSI")
...
Public Class MenuDOTitipanSupermarketPerwakilan
    ...
    Private Sub GetData()
        TBMenu.DataSource = SelectCon.execute("SELECT A.ID_TRANSAKSI,A.NO_DO,A.TGL,A.TGL_PENGAKUAN,B.NAMA,D.NAMA_USER,A.TGL_PRICE,DPP+PPN AS TOTAL,A.BATAL FROM DELIVERY_ORDER A WITH (NOLOCK) LEFT JOIN CUSTOMER B WITH (NOLOCK) ON A.KODE_CUSTOMER=B.ID LEFT JOIN LINK_USER_DIVISI C WITH (NOLOCK) ON A.DIVISI=C.KODE_DIVISI LEFT JOIN USERS D WITH (NOLOCK) ON A.CRUSER=D.ID_USER WHERE BAGIAN='Supermarket Perwakilan' AND JENIS_DO='Tanpa Barang' AND PERIODE='" & periode & "'  AND C.ID_USER='" & UserID & "'")
        'TBMenu.DataSource = SelectCon.execute("SELECT A.ID_TRANSAKSI,A.NO_DO,A.TGL,A.TGL_PENGAKUAN,B.NAMA,D.NAMA_USER,A.TGL_PRICE,DPP+PPN AS TOTAL,A.BATAL FROM DELIVERY_ORDER A WITH (NOLOCK) INNER JOIN CUSTOMER B WITH (NOLOCK) ON A.KODE_CUSTOMER=B.ID INNER JOIN LINK_USER_DIVISI C WITH (NOLOCK) ON A.DIVISI=C.KODE_DIVISI INNER JOIN USERS D WITH (NOLOCK) ON A.CRUSER=D.ID_USER WHERE BAGIAN='Supermarket Perwakilan' AND JENIS_DO='Tanpa Barang' AND PERIODE='" & periode & "'  AND C.ID_USER='" & UserID & "'")
        TBMenu.Refresh()
        Frame1.Text = "[ " & GridView1.DataRowCount & " Data Delivery Order ]"
        GridView1.Columns(0).Caption = "ID Transaksi"
        GridView1.Columns(0).Width = 30
...
Public Class MenuBonPesananSupermarketPerwakilan
    ...
    Private Sub GetData()
        TBMenu.DataSource = SelectCon.execute("SELECT A.ID_TRANSAKSI,A.NO_BON,A.TGL,A.TGL_PENGAKUAN,A.NO_DO,A.TGL_PRICE,B.NAMA,D.NAMA_USER,DPP+PPN AS TOTAL,A.BATAL FROM BON_PESANAN A WITH (NOLOCK) LEFT JOIN CUSTOMER B WITH (NOLOCK) ON A.KODE_CUSTOMER=B.ID LEFT JOIN LINK_USER_DIVISI C ON A.DIVISI=C.KODE_DIVISI LEFT JOIN USERS D WITH (NOLOCK) ON A.CRUSER=D.ID_USER WHERE BAGIAN='Supermarket Perwakilan' AND PERIODE='" & periode & "' AND C.ID_USER='" & UserID & "'")
        'TBMenu.DataSource = SelectCon.execute("SELECT A.ID_TRANSAKSI,A.NO_BON,A.TGL,A.TGL_PENGAKUAN,A.NO_DO,A.TGL_PRICE,B.NAMA,D.NAMA_USER,DPP+PPN AS TOTAL,A.BATAL FROM BON_PESANAN A WITH (NOLOCK) INNER JOIN CUSTOMER B WITH (NOLOCK) ON A.KODE_CUSTOMER=B.ID INNER JOIN LINK_USER_DIVISI C WITH (NOLOCK) ON A.DIVISI=C.KODE_DIVISI LEFT JOIN USERS D ON A.CRUSER=D.ID_USER WHERE BAGIAN='Supermarket Perwakilan' AND PERIODE='" & periode & "' AND C.ID_USER='" & UserID & "'")
        TBMenu.Refresh()
        Frame1.Text = "[ " & GridView1.DataRowCount & " Data Delivery Order ]"
        GridView1.Columns(0).Caption = "ID Transaksi"
        GridView1.Columns(0).Width = 30
...
Public Class MenuStuffingSupermarketPerwakilan
    ...
    Private Sub GetData()
        TBMenu.DataSource = SelectCon.execute("SELECT ID_TRANSAKSI,NO_STUFFING,TGL,NO_DO,TGL_DO,NAMA,D.NAMA_USER,A.BATAL FROM STUFFING A WITH (NOLOCK) LEFT JOIN CUSTOMER B WITH (NOLOCK) ON A.KODE_CUSTOMER=B.ID LEFT JOIN LINK_USER_DIVISI C WITH (NOLOCK) ON A.DIVISI=C.KODE_DIVISI LEFT JOIN USERS D WITH (NOLOCK) ON A.CRUSER=D.ID_USER WHERE BAGIAN='Supermarket Perwakilan' AND PERIODE='" & periode & "' AND C.ID_USER='" & UserID & "' ORDER BY TGL")
        'TBMenu.DataSource = SelectCon.execute("SELECT ID_TRANSAKSI,NO_STUFFING,TGL,NO_DO,TGL_DO,NAMA,D.NAMA_USER,A.BATAL FROM STUFFING A WITH (NOLOCK) INNER JOIN CUSTOMER B WITH (NOLOCK) ON A.KODE_CUSTOMER=B.ID INNER JOIN LINK_USER_DIVISI C WITH (NOLOCK) ON A.DIVISI=C.KODE_DIVISI INNER JOIN USERS D WITH (NOLOCK) ON A.CRUSER=D.ID_USER WHERE BAGIAN='Supermarket Perwakilan' AND PERIODE='" & periode & "' AND C.ID_USER='" & UserID & "' ORDER BY TGL")
        TBMenu.Refresh()
...
Public Class MenuNotaSJSupermarketPerwakilan
    ...
    Private Sub Cetak() Handles BtnCetak.ItemClick
        ...
        If SelectCon.execute("SELECT SJ_TANPA_HARGA FROM NOTA WITH (NOLOCK) WHERE ID_TRANSAKSI='" & GridView1.GetFocusedRow(0) & "'")(0)(0) Then
