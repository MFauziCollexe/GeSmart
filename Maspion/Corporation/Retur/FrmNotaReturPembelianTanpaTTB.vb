...
Public Class InputNotaReturPembelianTanpaTTB
    ...
    Private Sub Generate()
        Dim urut As Short
        Using dtGenerate = SelectCon.execute("SELECT NO_RETUR FROM RETUR_PEMBELIAN WITH (NOLOCK) WHERE NO_RETUR Like '" & TxtNamaDivisi.Text & "%' AND YEAR(TGL_PENGAKUAN)=" & Format(TglPengakuan.EditValue, "yyyy") & " AND DIVISI='" & TxtDivisi.Text & "' ORDER BY NO_RETUR DESC")
        ...
        Using dtGenerate = SelectCon.execute("SELECT ISNULL(MAX(CAST(REPLACE(ID_TRANSAKSI,'RB','') AS INT)),0) AS ID FROM RETUR_PEMBELIAN WITH (NOLOCK)")
