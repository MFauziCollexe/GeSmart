...
Public MustInherit Class FrmNotaReturPenjualan
    ...
    Private Sub TxtIDCustomer_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles TxtIDCustomer.EditValueChanged
        ...
        Using dtCustomer = SelectCon.execute("SELECT NO_NPWP FROM CUSTOMER WITH (NOLOCK) WHERE ID='" & TxtIDCustomer.Text & "'")
        ...
        bg = SelectCon.execute("SELECT CASE WHEN GROUP_CUSTOMER='Distributor' THEN 'Langganan' ELSE GROUP_CUSTOMER END AS GRUP FROM CUSTOMER WITH (NOLOCK) WHERE ID='" & TxtIDCustomer.Text & "'").Rows(0).Item(0)
    End Sub
    ...
    Private Sub TxtIDTTB_EditValueChanged(sender As Object, e As System.EventArgs) Handles TxtIDTTB.EditValueChanged
        If Status_Edit = False Then
            LoadData.GetData("SELECT NO_NOTA,TGL_PENGAKUAN,DIVISI,KODE_CUSTOMER,KODE_APPROVE,ALAMAT,GUDANG,KETERANGAN_CETAK FROM TTB WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDTTB.Text & "'")
    ...
    Private Sub BtnRubahHarga_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BtnRubahHarga.ItemClick
        Using LoadDataToGrid As New _LoadDataToGrid
            LoadDataToGrid.BeginInit("SELECT 'Pilih' AS PILIH,HARGA_BARU,TGL_PRICE FROM VI_PRICE_LIST WITH (NOLOCK) WHERE ID_BARANG='" & GridView1.GetFocusedDataRow()("ID Barang") & "' AND JENIS='" & bg & "' AND (ID='UMUM' OR ID='" & TxtIDCustomer.Text & "') ORDER BY TGL_PRICE DESC")
...
Public Class InputNotaReturPenjualan
    ...
    Private Function Generate() As Boolean
        Dim urut As Short
        Using dtGenerate = SelectCon.execute("SELECT NO_NOTA_RETUR FROM RETUR_PENJUALAN WITH (NOLOCK) WHERE NO_NOTA_RETUR Like '" & TxtNamaDivisi.Text & "(" & InisialPerusahaan & ")" & "%' AND YEAR(TGL_PENGAKUAN)=" & Format(TglPengakuan.EditValue, "yyyy") & " AND DIVISI='" & TxtDivisi.Text & "' ORDER BY NO_NOTA_RETUR DESC")
        ...
        Using dtGenerate = SelectCon.execute("SELECT ISNULL(MAX(CAST(REPLACE(ID_TRANSAKSI,'RTR','') AS INT)),0) AS ID FROM RETUR_PENJUALAN WITH (NOLOCK)")
    ...
    Private Sub Simpan() Handles _Toolbar1_Button1.ItemClick
        ...
        Using SQLServer As New SQLServerTransaction
            ...
            'Detail
            If SQLServer.InsertDetail(dt.Select("[Kode Barang]<>'' AND Pieces<>0").CopyToDataTable, {ToObject(TxtIDTransaksi.Text), ToObject(TxtNoTransaksi.Text), "ID Barang", "Isi", "Koli", "Kuantum", "Satuan", "Konv", "Pieces", "Harga Satuan", "Disc %", "Disc Satuan", "No."}, "DETAIL_RETUR_PENJUALAN") = False Then Exit Sub
            If SQLServer.InsertDetail(dt.Select("[Kode Barang]<>'' AND Pieces<>0").CopyToDataTable, {ToObject(TxtIDTransaksi.Text), ToObject(TxtNoTransaksi.Text), ToObject(Format(TglTransaksi.DateTime, "MM/dd/yyyy")), ToObject(Format(TglPengakuan.DateTime, "MM/dd/yyyy")), ToObject(TxtKodeGudang.Text), "ID Barang", "Kode Barang", "Pieces", "Satuan", "Harga Satuan", ToObject(periode)}, "SALDO_STOK", "[ID_TRANSAKSI] ,[NO_TRANSAKSI] ,[TGL_TRANSAKSI] ,[TGL_PENGAKUAN] ,[GUDANG] ,[ID_BARANG] ,[KODE_BARANG] ,[QUANTITY] ,[SATUAN] ,[HARGA] ,[PERIODE]") = False Then Exit Sub
