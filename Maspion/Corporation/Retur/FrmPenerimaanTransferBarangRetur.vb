   ...
   Private Sub TxtNoTransfer_KeyPress(sender As Object, e As System.Windows.Forms.KeyPressEventArgs) Handles TxtNoTransfer.KeyPress
    ...
    Private Sub TxtIDTransfer_EditValueChanged(sender As Object, e As System.EventArgs) Handles TxtIDTransfer.EditValueChanged
        If Status_Edit = False Then
            LoadData.GetData("SELECT NO_TRANSAKSI,DIVISI,GUDANG_SUMBER,GUDANG_TUJUAN FROM TRANSFER_BARANG_RETUR WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDTransfer.Text & "'")
    ...
    Private Sub TxtNoTransfer_EditValueChanged(sender As Object, e As EventArgs) Handles TxtNoTransfer.EditValueChanged
...
Public Class InputPenerimaanTransferBarangRetur
    ...
    Private Function Generate() As Boolean
        Dim urut As Short
        Using dtGenerate = SelectCon.execute("SELECT NO_TRANSAKSI FROM PENERIMAAN_TRANSFER_BARANG_RETUR WITH (NOLOCK) WHERE NO_TRANSAKSI Like '" & TxtNamaDivisi.Text & "%' AND YEAR(TGL_PENGAKUAN)=" & Format(TglPengakuan.EditValue, "yyyy") & " AND DIVISI='" & TxtDivisi.Text & "' ORDER BY NO_TRANSAKSI DESC")
        ...
        Using dtGenerate = SelectCon.execute("SELECT ISNULL(MAX(CAST(REPLACE(ID_TRANSAKSI,'TBR','') AS INT)),0) AS ID FROM PENERIMAAN_TRANSFER_BARANG_RETUR WITH (NOLOCK)")
...
Public Class EditPenerimaanTransferBarangRetur
    ...
    Private Sub GetData() Handles _Toolbar1_Button2.ItemClick, TxtIDTransaksi.EditValueChanged
        LoadData.GetData("SELECT NO_TRANSAKSI ,[TGL] ,[TGL_PENGAKUAN] ,ID_TRANSFER,NO_TRANSFER,[DIVISI] ,[GUDANG_SUMBER] ,[GUDANG_TUJUAN] ,[KETERANGAN_CETAK] ,[KETERANGAN_INTERNAL] FROM PENERIMAAN_TRANSFER_BARANG_RETUR WITH (NOLOCK) WHERE ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'")
