...
Public Class MenuNotaReturPenjualan
    ...
    Private Sub GetData()
        TBMenu.DataSource = SelectCon.execute("SELECT A.ID_TRANSAKSI,A.NO_NOTA_RETUR,A.TGL,B.NAMA,A.NO_TTB,A.KETERANGAN_CETAK,C.NAMA_USER,DPP+PPN AS TOTAL,A.BATAL FROM RETUR_PENJUALAN A LEFT JOIN CUSTOMER B ON A.KODE_CUSTOMER=B.ID LEFT JOIN USERS C ON A.CRUSER=C.ID_USER LEFT JOIN LINK_USER_DIVISI D ON A.DIVISI=D.KODE_DIVISI WHERE PERIODE='" & periode & "' AND D.ID_USER='" & UserID & "'")
        TBMenu.Refresh()
        ...
        GridView1.Columns(3).Width = 75
        GridView1.Columns(4).Caption = "No. TTB"
...
Public Class MenuTransferBarangRetur
    ...
    Private Sub GetData()
        TBMenu.DataSource = SelectCon.execute("SELECT DISTINCT A.ID_TRANSAKSI,A.NO_TRANSAKSI,A.TGL,E.NAMA_GUDANG,F.NAMA_GUDANG,STUFF((SELECT DISTINCT ';' + X.NO_TTB FROM DETAIL_TRANSFER_BARANG_RETUR X WHERE X.ID_TRANSAKSI=A.ID_TRANSAKSI FOR XML PATH('')), 1, 1, ''),D.NAMA_USER,A.BATAL FROM TRANSFER_BARANG_RETUR A CROSS JOIN PERUSAHAAN B LEFT JOIN CUSTOMER C ON B.CUST_PEMBELIAN=C.ID LEFT JOIN USERS D ON A.CRUSER=D.ID_USER LEFT JOIN GUDANG E ON A.GUDANG_SUMBER=E.KODE LEFT JOIN GUDANG F ON A.GUDANG_TUJUAN=F.KODE LEFT JOIN LINK_USER_DIVISI G ON A.DIVISI=G.KODE_DIVISI WHERE PERIODE='" & periode & "' AND G.ID_USER='" & UserID & "'")
        TBMenu.Refresh()
        Frame1.Text = "[ " & GridView1.DataRowCount & " Data Purchase Order ]"
        GridView1.Columns(0).Caption = "ID Transaksi"
        GridView1.Columns(0).Width = 40
...
Public Class MenuPenerimaanTransferBarangRetur
    ...
    Private Sub GetData()
        TBMenu.DataSource = SelectCon.execute("SELECT DISTINCT A.ID_TRANSAKSI,A.NO_TRANSAKSI,A.TGL,A.NO_TRANSFER,E.NAMA_GUDANG,F.NAMA_GUDANG,STUFF((SELECT DISTINCT ';' + X.NO_TTB FROM DETAIL_PENERIMAAN_TRANSFER_BARANG_RETUR X WHERE X.ID_TRANSAKSI=A.ID_TRANSAKSI FOR XML PATH('')), 1, 1, ''),D.NAMA_USER,A.BATAL FROM PENERIMAAN_TRANSFER_BARANG_RETUR A CROSS JOIN PERUSAHAAN B LEFT JOIN CUSTOMER C ON B.CUST_PEMBELIAN=C.ID LEFT JOIN USERS D ON A.CRUSER=D.ID_USER LEFT JOIN GUDANG E ON A.GUDANG_SUMBER=E.KODE LEFT JOIN GUDANG F ON A.GUDANG_TUJUAN=F.KODE LEFT JOIN LINK_USER_DIVISI G ON A.DIVISI=G.KODE_DIVISI WHERE PERIODE='" & periode & "' AND G.ID_USER='" & UserID & "'")
        TBMenu.Refresh()
        Frame1.Text = "[ " & GridView1.DataRowCount & " Data Purchase Order ]"
        GridView1.Columns(0).Caption = "ID Transaksi"
        GridView1.Columns(0).Width = 40
